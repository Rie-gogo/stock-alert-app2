/**
 * B2方式 対称化テスト: 9:30 bearish → 前場 LONG medium ブロック
 * 
 * 比較パターン:
 *   A: 現行（BUY medium全ブロック + 前場bullish時SHORT mediumブロック）
 *   B: 対称化（前場bullish→SHORT mediumブロック + 前場bearish→LONG mediumブロック、それ以外のmediumは許可）
 *   C: LONG medium全ブロック維持 + bearish時LONG strong閾値引き上げ（参考）
 * 
 * 共通条件:
 *   - 10銘柄限定
...
type PatternMode = "current" | "symmetric" | "long_block_enhanced";
      ...
      for (let i = 0; i < signals.length; i++) {
        ...
        if (inLongPosition && longEntry) {
          ...
          if (gain >= TP_PERCENT) {
            ...
            const entrySession: "am" | "pm" = candles[longEntry.idx].time.split(":").map(Number)[0] * 60 + parseInt(candles[longEntry.idx].time.split(":")[1]) < 12 * 60 + 30 ? "am" : "pm";
            trades.push({ date, symbol, side: "long", entryTime: candles[longEntry.idx].time,
              ...
              signalReason: longEntry.reason, confidence: longEntry.conf, beTriggered: longEntry.beActive, session: entrySession });
        ...
        if (inShortPosition && shortEntry) {
          ...
          if (gain >= TP_PERCENT) {
            ...
            const entrySession: "am" | "pm" = candles[shortEntry.idx].time.split(":").map(Number)[0] * 60 + parseInt(candles[shortEntry.idx].time.split(":")[1]) < 12 * 60 + 30 ? "am" : "pm";
            trades.push({ date, symbol, side: "short", entryTime: candles[shortEntry.idx].time,
              ...
              signalReason: shortEntry.reason, confidence: shortEntry.conf, beTriggered: shortEntry.beActive, session: entrySession });
        ...
        // ===== Medium フィルター（パターン別） =====
        if (mode === "current") {
          // 現行: BUY medium全ブロック + 前場bullish時SHORT mediumブロック
          if (confidence === "medium" && sig.type === "buy") continue;
          if (confidence === "medium" && sig.type === "sell") {
            if (isAM && direction === "bullish") continue;
          }
        } else if (mode === "symmetric") {
          // 対称化: 
          //   前場bullish → SHORT mediumブロック
          //   前場bearish → LONG mediumブロック
          //   neutral → 両方許可
          //   後場は両方無条件許可
          if (confidence === "medium" && sig.type === "sell") {
            if (isAM && direction === "bullish") continue;
          }
          if (confidence === "medium" && sig.type === "buy") {
            if (isAM && direction === "bearish") continue;
            // neutral/bullish前場 → LONG medium許可
            // 後場 → LONG medium許可
          }
        } else if (mode === "long_block_enhanced") {
          // 強化版: BUY medium全ブロック維持 + bearish時はBUY strong板スコア閾値引き上げ（参考）
          if (confidence === "medium" && sig.type === "buy") continue;
          if (confidence === "medium" && sig.type === "sell") {
            if (isAM && direction === "bullish") continue;
          }
          // bearish時のLONG strongはBPR条件追加
          if (sig.type === "buy" && direction === "bearish" && isAM) {
            const bpr = bprs[i];
            if (bpr < 0.6) continue; // 買い圧力が弱いならLONG strongもブロック
          }
        }
...
function printReport(title: string, trades: Trade[]) {
  ...
  // LONG medium vs strong breakdown
  const longMedium = longTrades.filter(t => t.confidence === "medium");
  const longStrong = longTrades.filter(t => t.confidence === "strong");
  const longMedPnl = longMedium.reduce((s, t) => s + t.pnl, 0);
  const longStrPnl = longStrong.reduce((s, t) => s + t.pnl, 0);
  ...
  console.log(`    LONG medium: ${longMedium.length}件 | ${longMedPnl >= 0 ? "+" : ""}${longMedPnl.toFixed(0)}円`);
  console.log(`    LONG strong: ${longStrong.length}件 | ${longStrPnl >= 0 ? "+" : ""}${longStrPnl.toFixed(0)}円`);
  ...
  return { count: trades.length, winRate, totalPnl, pf, maxDD, expectancy, longPnl, shortPnl, amPnl, pmPnl, longMedPnl, longStrPnl, longMedCount: longMedium.length, longStrCount: longStrong.length };
}
...
async function main() {
  ...
  console.log("=== B2方式 対称化テスト: bearish時 前場LONG mediumブロック ===");
  ...
  for (const date of dates) {
    ...
    for (const [symbol, symbolRows] of Array.from(bySymbol.entries())) {
      ...
      const enrichedCandles = symbolRows.map((r: any, i: number) => ({
        ...
        ma5: ma5[i] ?? closes[i], ma25: ma25[i] ?? closes[i],
        rsi: rsiArr[i] ?? 50, atr: null as any,
      }));
  ...
  for (const date of dates) {
    directionCache.set(`${date}_570`, getMarketDirection(candleCache, date, 570));
  }

  // Print direction
  console.log("--- 9:30方向性判定 ---");
  for (const date of dates) {
    const dir = directionCache.get(`${date}_570`) || "neutral";
    console.log(`  ${date}: ${dir}`);
  }
  console.log();

  // Run 3 patterns
  const tradesA = runSimulation(dates, "current", signalCache, candleCache, directionCache, bprCache);
  const tradesB = runSimulation(dates, "symmetric", signalCache, candleCache, directionCache, bprCache);
  const tradesC = runSimulation(dates, "long_block_enhanced", signalCache, candleCache, directionCache, bprCache);

  const resultA = printReport("A: 現行（BUY medium全ブロック + bullish時SHORT mediumブロック）", tradesA);
  const resultB = printReport("B: 対称化（bullish→SHORT mediumブロック / bearish→LONG mediumブロック）", tradesB);
  const resultC = printReport("C: BUY medium全ブロック維持 + bearish時LONG strong BPR<0.6ブロック", tradesC);
  ...
  console.log(`\n| パターン | LONG損益 | SHORT損益 | 前場損益 | 後場損益 | LONG med | LONG str |`);
  console.log(`|----------|----------|----------|----------|----------|---------|---------|`);
  for (const r of results) {
    console.log(`| ${r.name} | ${r.longPnl >= 0 ? "+" : ""}${r.longPnl.toFixed(0)}円 | ${r.shortPnl >= 0 ? "+" : ""}${r.shortPnl.toFixed(0)}円 | ${r.amPnl >= 0 ? "+" : ""}${r.amPnl.toFixed(0)}円 | ${r.pmPnl >= 0 ? "+" : ""}${r.pmPnl.toFixed(0)}円 | ${r.longMedCount}件/${r.longMedPnl >= 0 ? "+" : ""}${r.longMedPnl.toFixed(0)}円 | ${r.longStrCount}件/${r.longStrPnl >= 0 ? "+" : ""}${r.longStrPnl.toFixed(0)}円 |`);
  }

  // Bearish day analysis
  console.log(`\n--- bearish日の影響分析 ---`);
  const bearishDates = dates.filter(d => directionCache.get(`${d}_570`) === "bearish");
  console.log(`bearish日: ${bearishDates.join(", ")}`);
  for (const date of bearishDates) {
    const dayA = tradesA.filter(t => t.date === date);
    const dayB = tradesB.filter(t => t.date === date);
    const longA = dayA.filter(t => t.side === "long");
    const longB = dayB.filter(t => t.side === "long");
    const longMedA = longA.filter(t => t.confidence === "medium");
    const longMedB = longB.filter(t => t.confidence === "medium");
    console.log(`  ${date}:`);
    console.log(`    A: LONG ${longA.length}件(med ${longMedA.length}件) | ${longA.reduce((s, t) => s + t.pnl, 0).toFixed(0)}円`);
    console.log(`    B: LONG ${longB.length}件(med ${longMedB.length}件) | ${longB.reduce((s, t) => s + t.pnl, 0).toFixed(0)}円`);
    // Show blocked trades
    const blockedLongMed = longMedA.filter(t => {
      const [h2, m2] = t.entryTime.split(":").map(Number);
      return h2 * 60 + m2 < 11 * 60 + 30;
    });
    if (blockedLongMed.length > 0) {
      console.log(`    ブロック対象(前場LONG med): ${blockedLongMed.length}件 | 損益: ${blockedLongMed.reduce((s, t) => s + t.pnl, 0).toFixed(0)}円`);
    }
  }
