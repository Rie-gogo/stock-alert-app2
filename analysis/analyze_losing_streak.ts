/**
 * analyze_losing_streak.ts
 * 2026/1/30〜2/5 の連続マイナス期間の取引詳細を分析する
 */
...
const TARGET_DATES = ["2026-01-30", "2026-02-02", "2026-02-03", "2026-02-04", "2026-02-05"];
const INITIAL_EQUITY = 5_000_000;
const LEVERAGE = 3;
const MAX_CONCURRENT_STOCKS = 3;
const capitalPerStock = (INITIAL_EQUITY * LEVERAGE) / MAX_CONCURRENT_STOCKS; // 500万×3÷3=500万
...
function main() {
  ...
  console.log("2026/1/30〜2/5 連続マイナス期間 詳細分析");
  console.log("=".repeat(60));
  ...
  for (const day of TARGET_DATES) {
    ...
    let dayProfit = 0;
    const stockResults: Array<{ name: string; profit: number; trades: Array<{ type: string; time: string; price: number; shares: number; profit?: number; reason?: string }> }> = [];
    ...
    for (const s of TARGET_STOCKS) {
      ...
      const res = simulateStockReal(
        ...
        capitalPerStock, 70, 30, 2.0, rangeBound, 1.0,
        { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitAllMinute: LUNCH_EXIT_ALL_MINUTE }
      );
      if (!res || res.tradesCount === 0) continue;
      dayProfit += res.profitAmount;

      // 取引詳細（エントリー・決済のペア）
      const trades: Array<{ type: string; time: string; price: number; shares: number; profit?: number; reason?: string }> = [];
      for (const t of res.trades) {
        const sig = (res.signals ?? []).find(sg => sg.time === t.time);
        trades.push({ type: t.type, time: t.time, price: t.price, shares: t.shares, profit: t.profit, reason: sig?.reason });
      }
      stockResults.push({ name: s.name, profit: res.profitAmount, trades });
    }

    // 市場全体の動き
    const mktStart = marketBiasByProgress(0);
    const mktEnd = marketBiasByProgress(1);
    const mktChange = (mktEnd * 100).toFixed(2);

    console.log(`\n【${day}】合計損益: ${dayProfit.toLocaleString()}円  市場効率: ${eff.toFixed(3)}  レンジ相場: ${rangeBound}  市場変化: ${mktChange}%`);
    console.log("-".repeat(50));

    // 銘柄別損益
    for (const sr of stockResults.sort((a, b) => a.profit - b.profit)) {
      const sign = sr.profit >= 0 ? "+" : "";
      console.log(`  ${sr.name}: ${sign}${sr.profit.toLocaleString()}円`);
      for (const t of sr.trades) {
        if (t.type === "sell" || t.type === "cover") {
          const sign2 = (t.profit ?? 0) >= 0 ? "+" : "";
          const reason = t.reason ? ` [${t.reason.substring(0, 30)}]` : "";
          console.log(`    ${t.time} ${t.type.padEnd(6)} ${t.shares}株 @${t.price}  損益: ${sign2}${(t.profit ?? 0).toLocaleString()}円${reason}`);
        } else {
          console.log(`    ${t.time} ${t.type.padEnd(6)} ${t.shares}株 @${t.price}`);
        }
      }
    }
  }
