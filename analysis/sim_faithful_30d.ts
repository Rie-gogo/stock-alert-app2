 ...
 * 
 * +D構成（6/26版回帰 + 後場BPRフィルター）:
 * - 全銘柄を時系列順にインターリーブ処理（銘柄別独立ではない）
 * - 1銘柄１ポジション制限（openPositions Map）
 * - MAX_TOTAL_EXPOSURE制限（891万円）
 * - ステートマシン３種（pullbackStates, roundLevelPendingStates, roundPullbackStates）
...
// B2方式撤廃 → isBullish方式に回帰（+D構成）

const TEN_SYMBOLS = ["8035","6857","6976","6526","5803","6981","285A","6146","6594","8316"];
...
async function main() {
  ...
  const [dateRows] = await db.execute(
    `SELECT tradeDate FROM (SELECT DISTINCT tradeDate FROM rt_candles WHERE tradeDate >= '2026-06-24' ORDER BY tradeDate DESC LIMIT 30) sub ORDER BY tradeDate`
  ) as any;
  ...
  for (const date of dates) {
    ...
    // isBullish方式: 銘柄別に始値を記録し、+0.2%以上で上昇判定
    ...
    for (const row of candles) {
      ...
      // isBullish: 銘柄別始値記録（最初の09:30以降の足）
      ...
      if (existingPos) {
        ...
        if (existingPos.side === "long") {
          const slPct = USE_PER_SYMBOL_SL ? (SYMBOL_SL_MAP[symbol] ?? STOP_LOSS_PERCENT) : STOP_LOSS_PERCENT;
          const stopLine = existingPos.entryPrice * (1 - slPct / 100);
          if (low <= stopLine) {
            exitPrice = stopLine;
            exitReason = "SL";
          ...
          const slPctS = USE_PER_SYMBOL_SL ? (SYMBOL_SL_MAP[symbol] ?? STOP_LOSS_PERCENT) : STOP_LOSS_PERCENT;
          const stopLine = existingPos.entryPrice * (1 + slPctS / 100);
          if (high >= stopLine) {
            exitPrice = stopLine;
            exitReason = "SL";
            exitAction = "stop_loss";
      ...
      // isBullish方式: 銘柄別に始値比で判定（symbolOpenPricesで管理）
      ...
      if (sig.type === "buy") {
        ...
        // 大台超え → LONG全面停止（buy_pressure時のみ逆張りSHORT）
        if (sig.reason.startsWith("大台超え")) {
          // buy_pressure判定: boardSignalで判定
          const boardSig = getBoardSignal(boardSnap);
          if (boardSig === "buy_pressure") {
            const entryResult = tryEnterPosition(
              "short", symbol, close, candleTime, date, "大台超え×buy_pressure逆張りSHORT",
              buffer, bpr, bprHist, openPositions, lastStopLossTime
            );
            if (entryResult) openPositions.set(symbol, entryResult);
          }
          continue; // LONG登録はスキップ
        }
        
        // BUY medium全ブロック（太陽誘電6976のGC mediumのみ許可: close>MA20 + 陽線条件）
        if (sig.confidence === "medium") {
          const isGcMedium6976 = symbol === "6976" && sig.reason.includes("GC") && close > (calcMA(buffer.map(b=>b.close), 20) ?? 0) && close > open;
          if (!isGcMedium6976) continue;
        }
      ...
      if (sig.type === "sell") {
        ...
        // isBullish方式: 銘柄別に始値比+0.2%以上なSHORT禁止
