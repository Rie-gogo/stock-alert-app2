...
async function main() {
  ...
  `) as any[];
  const targetDate = (dateRows as any[])[0].tradeDate;
  ...
  interface Trade { symbol: string; time: string; price: number; result: string; pnl: number; method: string; shares: number; exitTime: string; level: number; prevDist: number; volRatio: number; }
  ...
  for (const symbol of SYMBOLS) {
    ...
    for (let i = 2; i < candles.length; i++) {
      ...
      if (pendingState === "waiting") {
        ...
        if (pendingWait > 1) {
          const { result, pnl, shares, exitTime } = simulate(candles, i, symbol);
          trades.push({ symbol, time: candles[i].t, price: candles[i].c, result, pnl, method: "③従来(CB2MW1)", shares, exitTime, level: pendingLevel, prevDist: -1, volRatio: 0 });
      ...
      for (const level of levels) {
        if (prevClose >= level && currClose < level) {
          ...
          if (volRatio >= 1.5) {
            const { result, pnl, shares, exitTime } = simulate(candles, i, symbol);
            trades.push({ symbol, time: candles[i].t, price: candles[i].c, result, pnl, method: "①即vol", shares, exitTime, level, prevDist, volRatio });
            inPosition = true; break;
          }
          // ② 前足+0.05%以内
          if (prevDist <= 0.05) {
            const { result, pnl, shares, exitTime } = simulate(candles, i, symbol);
            trades.push({ symbol, time: candles[i].t, price: candles[i].c, result, pnl, method: "②即4a", shares, exitTime, level, prevDist, volRatio });
            inPosition = true; break;
          }
          // ③ 従来
  ...
  console.log(`${targetDate} 大台割れSHORT: 3方式シミュレーション`);
  console.log(`${"=".repeat(90)}\n`);

  console.log(`| # | 時刻 | 銘柄 | 方式 | エントリー | キリ番 | 前足乖離 | Vol倍率 | 決済(時刻) | 損益 |`);
  console.log(`|---|------|------|------|-----------|--------|----------|---------|-----------|------|`);
  let totalPnl = 0;
  ...
  // 本番結果取得
  const [realTrades] = await conn.query(`
    SELECT symbol, tradeTime, price, pnl, reason FROM rt_trades 
    WHERE tradeDate = '${targetDate}' AND action IN ('short','cover') AND side = 'short'
    ORDER BY tradeTime
  `) as any[];
  const realPnl = (realTrades as any[]).filter((r: any) => r.pnl !== null && r.pnl !== "NULL").reduce((s: number, r: any) => s + Number(r.pnl), 0);
  const realCount = (realTrades as any[]).filter((r: any) => r.action === 'short' || (r.pnl !== null && r.pnl !== "NULL")).length;

  console.log(`\n--- 本番実績との比較 ---`);
  console.log(`  本番SHORT損益: ${realPnl >= 0 ? "+" : ""}${realPnl.toLocaleString()}円`);
  console.log(`  3方式シミュ: ${totalPnl >= 0 ? "+" : ""}${totalPnl.toLocaleString()}円`);
  console.log(`  差分: ${(totalPnl - realPnl) >= 0 ? "+" : ""}${(totalPnl - realPnl).toLocaleString()}円`);
