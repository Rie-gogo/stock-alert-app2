...
// 現在のロジック全体で10営業日シミュレーション
// MA8, 3方式SHORT, 静かな上昇バイパス緩和A, SHORTスコア0緩和, 逆三尊バイパス除外, 前場強制決済11:27, 12:30-12:50禁止
...
async function main() {
  ...
  const [dates] = await db.execute(sql`
    SELECT DISTINCT tradeDate FROM rt_candles WHERE symbol = '8035' ORDER BY tradeDate DESC LIMIT 11
  `);
  const tradeDates = (dates as any[]).map(d => d.tradeDate).reverse();
  const simDates = tradeDates.slice(1); // 最初の1日はバッファ用
  ...
  for (const sym of SYMBOLS) {
    ...
    for (const date of simDates) {
      ...
      for (let i = 0; i < dayCandles.length; i++) {
        ...
        // 前場強制決済 11:27
        if (position && timeMin >= 687) { // 11:27
          if (timeMin < 750) { // 12:30前
        ...
        if (position) {
          ...
          const tpPrice = position.side === "short" ? position.price * (1 - 1.5/100) : position.price * (1 + 1.5/100);
        ...
        // エントリー判定
        if (timeMin < 570 || timeMin >= 905) continue; // 09:30前、15:05以降禁止
        ...
        if (i > 0 && buffer.length >= 2) {
          ...
          for (const rl of roundLevels) {
            ...
            if (prev.close >= nearestAbove && c.close < nearestAbove && (nearestAbove - c.close) / nearestAbove < 0.008) {
              // 大台割れ検出
              if (isBullish) break; // isBullishならSHORTブロック
        ...
        if (buffer.length >= 21 && !isBullish) {
          ...
          if (c.close < minLow) {
            // SHORTスコア0バイパス: ダウ理論はスコア0でもエントリー許可
            position = { side: "short", price: c.close, lots: LOTS[sym] || 100, time, method: "ダウ理論", signal: "ダウ理論SHORT" };
          }
        }
        if (position) continue;
        ...
        if (buffer.length >= 21) {
          ...
          if (c.close > maxHigh) {
            // 静かな上昇バイパス条件（緩和A）
            if (isBullish && buffer.length >= maPeriod) {
              const ma = buffer.slice(-maPeriod).reduce((s: number, b: any) => s + b.close, 0) / maPeriod;
              ...
              if (maDeviation < 0.5 && barBody < 0.2 && bearBars <= 4) {
                position = { side: "long", price: c.close, lots: LOTS[sym] || 100, time, method: "バイパスLONG", signal: "ダウ理論LONG" };
              }
            }
          }
        }
  ...
  console.log(`\n=== 全ロジック10営業日シミュレーション ===`);
  console.log(`全体: ${allTrades.length}件 ${wins.length}勝${losses.length}敗 勝率${(wins.length/allTrades.length*100).toFixed(1)}% ${totalPnl.toLocaleString()}円`);
  ...
  for (const d of simDates) {
    ...
    console.log(`  ${d}: ${dt.length}件 ${dp.toLocaleString()}円 (S:${ds.toLocaleString()} L:${dl.toLocaleString()})`);
  }
  
  // 方式別
  console.log(`\n--- 方式別 ---`);
  for (const m of [...new Set(allTrades.map(t => t.method))]) {
    const mt = allTrades.filter(t => t.method === m);
    console.log(`  ${m}: ${mt.length}件 ${mt.filter(t=>t.pnl>0).length}勝${mt.filter(t=>t.pnl<=0).length}敗 ${mt.reduce((s,t)=>s+t.pnl,0).toLocaleString()}円`);
  }
