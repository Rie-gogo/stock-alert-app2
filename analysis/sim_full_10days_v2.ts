...
// 現在の全ロジック10営業日シミュレーション v2
// 変更点: LONG TP=0.5%, 案A+B前場ブースト, 案1安値更新即, 案2大台割れisBullish免除
...
const SHORT_LOW_BREAK_VOL_RATIO = 1.2; // 案1
const AM_BOOST_MA_DEV_MAX = 1.0;       // 案A
const AM_BOOST_BODY_MAX = 0.5;         // 案A
const AM_BOOST_BEAR_MAX = 5;           // 案A
const AM_VOL_BREAK_RATIO = 1.5;        // 案B
const TP_SHORT = 1.5;                  // SHORT TP
const TP_LONG = 0.5;                   // LONG TP (変更: 1.5→0.5)
  ...
  for (const sym of SYMBOLS) {
    ...
    for (const date of simDates) {
      ...
      for (let i = 0; i < dayCandles.length; i++) {
        ...
        const isAM = timeMin < 688; // 11:28前 = 前場
