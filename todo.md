...
- [x] チェックポイント保存・自動公開後に本番DB、3版の独立性、初期状態、エラー0件を確認する
...
- [x] 8035改善案Aの正式開始日2026-09-07より前は評価日数0とし、API・16時報告へ混入させない

## 他AI監査指摘「P0/P1未完成点」精査（2026-09-04）
- [x] 証拠金ブロック時のrouteId・side・signal reason・wasMarginBlocked保存欠落をコードと本番DBで確認する
- [x] ブロック候補の経路固有仮想決済、全発火勝率、891万円portfolio損益が計算可能か確認する
- [x] 8035改善案Aのboard.currentPrice、best ask/bid、板鮮度、次イベント待機の仕様差を確認する
- [x] シャドー処理順・8035 parity比較項目・残り9銘柄の再現比較範囲を確認する
...
## 毎営業日16時途中経過報告・未設定経路棚卸し（2026-09-04）
- [x] 既存の16時日次報告handler・公開endpoint・スケジュール登録状態を確認する（既存アプリHeartbeatは本日timeoutのため、ユーザー選択によりManus AI報告へ分離）
- [x] 毎営業日16:00 JSTに現行・シャドー・P0監査の途中経過が通知される設定を重複なく作成または更新する
- [x] 16時報告が旧8035 A停止、depth新版、全候補台帳、仮想exit coverage、891万円portfolio、実時再生不一致を含むことを確認する
- [x] 現行10銘柄の全取引経路と既存シャドー案A/Bの対応を棚卸しする
- [x] 未設定経路を損益構造・TP/SL不適合・本番乖離影響・候補数に基づいて優先度順に整理する
- [x] 毎営業日16:00 JSTにManus上で新しいAI途中経過報告を作成する定期タスクを設定し、毎週月〜金16:00・activeを確認する
- [x] 定期AI報告へ現行損益、候補台帳、100株仮想取引、891万円portfolio、実時再生差、因果性、queue/error、候補別判定を含める
- [x] 現行10銘柄22経路を、2案設定済み2経路・1案のみ5経路・未設定15経路に分類した一覧表を作成する

## 他AI P0再監査（2026-09-04）
- [x] 本番同型の`result.reason=margin_block`で詳細シグナル理由が別保存される場合も、margin_block候補を構造化保存できるか再現確認する（`candidate_reason_missing`を再現）
- [x] 100株仮想決済が`usesSignalReversalExit`と`usesBoardEarlyExit`を使用し、現行専用出口と一致しているか全routeで監査する（両フラグ未使用を確認）
...
## 9984 前場10本高値更新LONG 改善案A/B設計（2026-09-05）
- [x] 現行9984前場10本高値更新LONGの入口・出口・日次枠・板・証拠金・TP/SL仕様を確認する
- [x] 保存KABU足と既存監査資料から、過去と本番の乖離に関係する因果的な負け方を整理する（44日25件＋2026-08-31〜09-04の5件）
- [x] 同じ条件の微調整ではなく、A=次イベント板depth実行確認、B=現行入口維持の2R＋利益保護として原理が異なる2案を具体化する
- [x] TPがSLの2倍以上、未来情報不使用、100株版と891万円版の分離、経路parity必須、条件途中変更禁止を固定する
- [x] 現行を変更せず、各案を別strategyVersion・別state・別取引・注文非接続の未見シャドー候補として提案する（実装は未実施）

## 9984 前場10本高値更新LONG 改善案A/Bシャドー実装（2026-09-05承認）
- [x] A案`forward-shadow-9984-breakout-depth-confirm-v1`をask価格昇順100株depth VWAP・板数量・ブレイク継続・0.10%価格悪化・同一時計5秒鮮度で実装する
- [x] A案をSL0.4%／TP0.8%／45分決済、拒否時日次枠非消費・元初動再利用禁止として独立状態機械化する
- [x] B案`forward-shadow-9984-breakout-rr2-protect-v1`を現行入口維持・SL0.5%／TP1.0%・+0.20%到達後次足以降+0.05%利益保護として実装する
- [x] B案の窓下げ不利始値と`SL→準備済み利益保護→TP→新規準備→45分決済`の順序を固定する（純粋回帰6件成功）
- [x] A/Bを別strategyVersion・別state・別judgment/trade・signal_quality／capital_constrained別評価として既存engineSequence順queueへ接続する
- [x] 現行9984・A・Bを同一source eventへ独立配信し、完了済み版は再送時に二重処理しない（純粋＋永続回帰8件成功、片方失敗時再試行は統合障害注入で追加確認予定）
- [x] 9984経路parity Gate、4週10件Gate、平均利益÷平均損失0.8以上、TP到達率、PF・期待値・0.10%不利・891万円手動比較Gateを追加する（Gate回帰9件成功、自動採用なし）
...
## 6976 候補B・10本高値更新LONG 改善案A/B検証（2026-09-05）
- [x] 現行6976候補B・10本高値更新LONGの入口・出口・SL/TP・保有時間・日次枠・早期決済仕様を確認する
- [x] 保存KABU 1分足だけで現行を再生し、固定fixture13件と2026-09-01・09-04追加2件の勝敗と損益構造を確定する
- [x] 未来情報を使わず、A=確認足出来高1.40倍、B=同時点BPR1.30以上という異なる原理の改善案を固定する
- [x] A/Bを同一保存KABUデータで再生し、SL0.5%／TP1.0%、勝率、PF、期待値、平均利益損失比を比較する
- [x] 0.10%不利約定、近接閾値感応度、取引数減少、過学習危険度を確認する
- [x] 現行売買を変更せず、Bを第1候補、Aを第2候補として別strategyVersionでの未見シャドー固定仕様を報告する（実装は未実施）

## 6976 候補B LONG・他AI案との比較精査（2026-09-05）
- [x] ユーザー提示案の仕様・改善原理・使用データ・出口条件を確認する
- [x] 既存の出来高案・板需給案と、入口・出口・地合い・実行品質などの観点重複を整理する
- [x] 保存KABUデータで全案を同一足SL優先・窓不利・0.10%不利条件で比較し、入口案は拒否後の後続候補を含む実エンジン同型再生を行う
- [x] 勝率、PF、期待値、平均利益損失比、取引数、閾値感応度、過学習危険度を比較する
- [x] 最適2案を「同時点板BPR1.30以上・SL0.5%/TP1.0%」と「現行入口維持・SL0.8%/TP1.6%・+0.24%準備/+0.16%保護」に確定する（実装は未実施）
- [x] 旧出来高1.40倍案は、後続再探索込み実エンジン再生で10件2勝8敗・20.0%となるため以前の第2候補判断を撤回し不採用とする
- [x] ユーザー提示の始値比+1.5%・09:45〜10:30案はfixture70.0%を再現したが、09-01/09-04追加後66.67%、0.10%不利後58.33%となるため不採用とする

## 6976 候補B LONG・選定2案シャドー実装（2026-09-05承認）
- [x] 板需給案`candidate-6976-board-demand-bpr130-v1`を現行初動・確認維持、BPR1.30以上、大口売り壁なし、SL0.5%／TP1.0%、30分決済で独立状態機械化する
- [x] 利益保護案`candidate-6976-rr2-protect-v1`を現行入口維持、SL0.8%／TP1.6%、+0.24%準備、次足以降+0.16%保護、30分決済で独立状態機械化する
- [x] 板需給案は同時点boardSnapshotだけを使用し、板欠損・時刻因果・大口売り壁・BPR判定を監査保存する
- [x] 利益保護案は窓下げ不利始値と`SL→準備済み利益保護→TP→新規準備→30分決済`を固定する
- [x] 2案を別strategyVersion・別state・別judgment/trade・signal_quality／capital_constrained別評価としてengineSequence順queueへ接続する
- [x] 現行6976と2案を同一source eventへ独立配信し、片側失敗時も完了側・親現行処理を二重実行しない
- [x] 6976候補B LONG経路parity、4週10件、勝率70%、PF1.5、期待値+0.15R、0.10%不利、891万円portfolio Gateを追加する
- [x] 利益保護案へ実現平均利益÷平均損失0.8以上とTP到達率表示の追加Gateを設定する
...
## 6526確認型LONG A/Bシャドー候補精査（2026-09-05）
- [x] 現行`server/socionextConfirmedLong.ts`と提示A/Bの入口・確認・拒否後再探索・日次終了・出口仕様を照合する
- [x] 46日fixture＋2026-08-31〜09-04の保存KABU 1分足5日で現行・A・Bを拒否後状態遷移込みで再計算する
- [x] A/Bの閾値感度、0.05%・0.10%不利約定、PF、最大DD、実現平均利益損失比、保有時間、期待値R、最大連敗を比較する
- [x] TP>=SL×2、4週10件、`socionextConfirmedLong` route parity、891万円portfolio非悪化、修正前・選定用データ除外のGate整合を確認する
- [x] Bは条件付き探索用シャドー候補、Aは単一仕様で提示数値を再現できず現状不採用と判定し、追加5日後の成績と実装前固定仕様を報告書へ整理する

## 6526 A案の状態遷移訂正・再精査（2026-09-05）
- [x] A案の+0.25%条件を確認成功後ではなく初動候補検出時へ移し、弱い初動でその日の探索を終了する仕様を固定する
- [x] 正しいA案で46日14件11勝3敗・+11,037.87円・PF7.23・最大DD678.38円を再現し、B案の46日14件11勝3敗・+15,331.41円も再確認する
- [x] 本番DBの2026-08-31〜09-04保存足を加え、正しいA/Bで51日・直近5/10完全保存日・0.05/0.10%不利約定・期待値R・最大連敗を再計算する（A=16件11勝5敗68.75%、B=15件11勝4敗73.33%）
- [x] 追加5日を実装前にGit固定fixtureへ含める必要性と、公開前の選定用51日を正式未見成績へ含めない境界を確認する
- [x] 前回A案不採用理由の誤りを撤回し、Bは採用審査候補、Aは最新勝率Gate未達だが原因切り分け用診断シャドーとしてA/B並行可能と訂正報告へ整理する

## 6526確認型LONG A/B前向きシャドー実装（2026-09-05）
- [x] 2026-08-31〜09-04の6526保存KABU 1分足1,676本を46日fixtureへ追補固定し、ファイルSHA`ded0658b…`と正しいA/Bの51日期待取引・日次終了イベントを固定する
- [x] A案を現行初動条件＋初動時始値比+0.25%未満で日次終了、SL0.25%／TP0.50%／20分の独立純粋状態機械として実装する
- [x] B案を現行初動条件＋確認上昇率+0.075%未満で日次終了、SL0.35%／TP0.70%／20分の独立純粋状態機械として実装する
- [x] 両案の確認失敗same-candle再検出、SL優先、窓下げ不利始値、20分・前場越境決済、日次終了、JSON再起動復元を固定する
- [x] A/Bを短い別strategyVersion・別state・別event・別trade・2評価方式で永続化し、state lock・冪等性・variant/mode単位の片側失敗再試行を実装する
- [x] 同一6526 source eventをengineSequence永続queueからA/Bへ独立配信し、現行売買と注文経路へ接続しない
- [x] 両案を`socionextConfirmedLong` route parityへ紐付け、Bを採用審査候補、Aを最新51日勝率Gate未達の診断候補として表示する
- [x] 4週10件・勝率70%・PF1.5・期待値+0.15R・0.10%不利後プラス・891万円portfolio非悪化・自動採用禁止をGate化する
- [x] RuntimeIdentity・固定版再生・16時レポート・公開forward summaryへA/Bを独立統合する
- [x] 匿名fixture・46/51日固定再生・dispatch・API・report・Gate・冪等性・部分障害・注文非接続の回帰を追加する
...
## 3436前場SHORT 4案比較・最終2案選定（2026-09-06）
- [x] 既存の出来高1.10倍案・現行入口15分案と、新規の2本後下落継続案・ATR品質スコア案を同一の現行入口起点・約定・出口・再探索基準へ正規化する
- [x] 2本後下落継続案を初動ATR固定・確認足・次足始値・確認失敗日次終了で29日および追加5日に再現し、件数/勝敗/DDは一致するが損益/PFは提示値と不一致と確認する
- [x] ATR品質スコア案の4指標・2項目以上・次足始値継続・不合格日次終了を29日および追加5日に再現し、件数/DDは一致するが勝敗/損益/PFは提示値と不一致と確認する
...
## 6976後場SHORT 修正版A/B depth実行シミュレーション（2026-09-06）
- [x] 全保存期間と直近5営業日の6976同時点boardSnapshotについて、bid/ask depth・数量・clock-safe時刻のcoverageを確定する（`rt_candles`51日16,409本はdepth0、source eventは9/3・9/4の2日786件のみ）
- [x] A案をbid側100株depth VWAP、確認終値以下、元5本安値未満、不利差0.10%以内、SL0.8%／TP1.6%／45分で因果的に再生する
- [x] B案を二段階確認後のbid側100株depth VWAP、確認終値以下、不利差0.10%以内、SL1.0%／TP2.0%／30分で因果的に再生する
- [x] 両案の買戻しをask側100株depth VWAPで評価し、板欠損・古い・100株未満時はexit intentを保持して次イベントで再試行する
- [x] 厳密再生可能な9/3・9/4を検算し、A/Bとも0件・損益0円・未決済0、Aは9/4 13:02にbid VWAP9,370円>確認終値9,355円で下落継続なし拒否と確認する
- [x] 全51日と直近5日はdepth coverage不足で勝率・損益を正式算出不能、2日0件の勝率は0%ではなく算定不能とし、旧次足始値/確定終値近似を分離してチャット報告へ整理する

## 6976後場反転SHORT 新規改善案2案設計（2026-09-06）
- [x] 現行経路の2R不足、始値アンカー、ブレイク不発、次足始値非因果性、depth coverage制約を整理する
- [x] 保存KABU 1分足だけで全期間検証可能な「現行1本確認入口＋SL0.8%／TP1.6%／45分」の出口改善案を設計する
...
## 6981安値反転ブレイクLONG 新規改善案2案設計（2026-09-06）
- [x] 現行17候補、SHORT排他、現行1.0%/1.5%非2R、反転/板早期/前場出口、実行価格感度を整理する
- [x] 保存KABU 1分足52日で検証可能な「現行入口維持＋SL0.4%／TP0.8%／15分」を設計し、17件13勝4敗・76.47%・+57,610.40円・PF5.56を再計算する
...
## 2026-08-26以降 ロジック別取引成績集計（証拠金ブロック含む）
- [x] 現行candidate正本と100株仮想取引を2026-08-26以降で抽出し、accepted／margin_block／未決済／gapを照合する
...
## Plan D 停止現行11経路・シャドー専用追跡（2026-09-12）
- [x] 同期前の未保存2ファイルとPlan Dの重なりを記録し、動作コード競合がないことを差分で確認する
- [x] GitHub `main`の指定commit `cccb5b610b0d857f783fb0239f0336fba2db375d`を同期し、HEAD・origin/main・直系性を確認する
...
- [x] c64a4fedの取得可否、親版との差分、未保存変更との競合範囲を確認する（GitHub未pushのローカル専用commitで取得不能と報告済み）
- [x] 6976候補Bの30分板決済と5秒鮮度をc64a4fed上で再現・検証する（GitHub未取得のため未実施。後続mainの因果的depth修正で代替検証）
- [x] `calculateClockSafeBoardAge`の全利用経路とstrategyVersionを稼働・開始前・停止中に棚卸しする（後続mainの共通鮮度補正で確認済み）
- [x] 影響範囲、修正仕様、回帰条件、公開停止要否をc64a4fed正本で再判定する（後続mainの対象54回帰で代替確認済み）
- [x] 更新版の精査結果と実装開始に必要な条件を報告する（c64a4fedのpush非推奨と最新mainからの移植方針を報告済み）
