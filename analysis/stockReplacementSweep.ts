/**
 * stockReplacementSweep.ts
 * SIMULATION_STOCKSの銘柄入れ替えスイープ
 *
 * 現在のSIMULATION_STOCKS（10銘柄）のうち、HIGH_VOL_SYMBOLSに含まれる
 * 極小ロット銘柄（8306・6758・7011等）を除外し、TARGET_STOCKSの他の銘柄
 * （通常ロット）と入れ替えた場合の損益を比較する。
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/stockReplacementSweep.ts
 */
...
// 全銘柄の損益を計算する関数
function calcAllSymbolProfits(
  allDays: string[],
  byTicker: Map<string, Map<string, JqBar[]>>,
  symbols: string[]
): Map<string, number> {
  const profits = new Map<string, number>();
  for (const sym of symbols) profits.set(sym, 0);

  for (const day of allDays) {
    const candleMap = new Map<string, RealCandle[]>();
    for (const sym of symbols) {
      const bars = byTicker.get(sym)?.get(day);
      if (!bars || bars.length < 60) continue;
      candleMap.set(sym, barsToCandles(bars));
    }
    ...
    for (const sym of symbols) {
      ...
      if (!res) continue;
      profits.set(sym, (profits.get(sym) ?? 0) + res.profitAmount);
    }
  }
  return profits;
}
...
async function main() {
  ...
  // 全銘柄の損益を計算（全20銘柄を市場バイアス計算に使用）
  const allSymbols = TARGET_STOCKS.map(s => s.symbol);
  console.log(`\n[sweep] 全20銘柄の損益を計算中...`);
  const allProfits = calcAllSymbolProfits(allDays, byTicker, allSymbols);

  // 結果を損益順にソート
  const sortedSymbols = allSymbols
    .map(sym => {
      const s = TARGET_STOCKS.find(x => x.symbol === sym)!;
      return { symbol: sym, name: s.name, sector: s.sector, profit: allProfits.get(sym) ?? 0 };
    })
    .sort((a, b) => b.profit - a.profit);

  console.log("\n=== 全20銘柄 損益ランキング（通常ロット強制） ===");
  console.log(`${"順".padEnd(3)} ${"銘柄".padEnd(26)} ${"セクター".padEnd(14)} ${"損益".padStart(12)}`);
  console.log("-".repeat(60));
  for (let i = 0; i < sortedSymbols.length; i++) {
    const r = sortedSymbols[i];
    ...
    console.log(`${rank} ${name} ${sector} ${profit}`);
  }

  // 現在のSIMULATION_STOCKS（10銘柄）
  const currentSim = ['3436', '3778', '6981', '6758', '8306', '8035', '6857', '6920', '7011', '9984'];
  const currentTotal = currentSim.reduce((sum, sym) => sum + (allProfits.get(sym) ?? 0), 0);
  console.log(`\n現在のSIMULATION_STOCKS（10銘柄）合計: ${currentTotal >= 0 ? "+" : ""}${Math.round(currentTotal).toLocaleString()}円`);

  // 上位10銘柄を選んだ場合の合計
  const top10 = sortedSymbols.slice(0, 10);
  const top10Total = top10.reduce((sum, r) => sum + r.profit, 0);
  console.log(`\n=== 上位10銘柄を選んだ場合 ===`);
  for (const r of top10) {
    const inCurrent = currentSim.includes(r.symbol) ? "★" : "NEW";
    console.log(`  ${inCurrent} ${r.symbol} ${r.name}: ${r.profit >= 0 ? "+" : ""}${Math.round(r.profit).toLocaleString()}円`);
  }
  console.log(`上位10銘柄合計: ${top10Total >= 0 ? "+" : ""}${Math.round(top10Total).toLocaleString()}円`);
  console.log(`差分: ${top10Total - currentTotal >= 0 ? "+" : ""}${Math.round(top10Total - currentTotal).toLocaleString()}円`);

  // 段階的な入れ替えシミュレーション
  console.log("\n=== 段階的入れ替えシミュレーション ===");
  const nonCurrentTop = sortedSymbols.filter(r => !currentSim.includes(r.symbol));
  const currentWorst = [...currentSim]
    .map(sym => ({ symbol: sym, profit: allProfits.get(sym) ?? 0 }))
    .sort((a, b) => a.profit - b.profit);

  console.log("\n現在のSIMULATION_STOCKS（損益ワースト順）:");
  for (const r of currentWorst) {
    const s = TARGET_STOCKS.find(x => x.symbol === r.symbol)!;
    console.log(`  ${r.symbol} ${s.name}: ${r.profit >= 0 ? "+" : ""}${Math.round(r.profit).toLocaleString()}円`);
  }

  console.log("\n入れ替え候補（現在のSIMに含まれない銘柄、損益上位）:");
  for (const r of nonCurrentTop.slice(0, 10)) {
    console.log(`  ${r.symbol} ${r.name}: ${r.profit >= 0 ? "+" : ""}${Math.round(r.profit).toLocaleString()}円`);
  }

  // 最悪銘柄を1つずつ入れ替えた場合の差分
  console.log("\n1銘柄入れ替えシミュレーション（最悪銘柄 → 最良候補）:");
  for (let i = 0; i < Math.min(5, currentWorst.length); i++) {
    const out = currentWorst[i];
    const outS = TARGET_STOCKS.find(x => x.symbol === out.symbol)!;
    for (let j = 0; j < Math.min(3, nonCurrentTop.length); j++) {
      const inR = nonCurrentTop[j];
      const diff = inR.profit - out.profit;
      if (diff > 0) {
        console.log(`  ${out.symbol}(${outS.name}, ${Math.round(out.profit).toLocaleString()}円) → ${inR.symbol}(${inR.name}, ${Math.round(inR.profit).toLocaleString()}円): +${Math.round(diff).toLocaleString()}円`);
      }
    }
  }
}
