/**
 * 3山切り下げエントリー v2 (SHORT) - 村田製作所(6981)専用
 * 
 * 【条件】
 ...
 *    - 陰線転換（前足陽線→今足陰線）
 *    - 次足で前足安値更新
 * ④ 【追加】直近スイングハイ以降、そのスイングハイを超える高値が出ていないこと
 *    （＝反発が前回の山を超えていない → 下降トレンド継続中を確認）
 * 
...
// 【追加条件④】直近スイングハイ以降に、そのスイングハイを超える高値が出ていないか
function isStillInDowntrend(candles, swingHighs, currentIdx) {
  ...
  for (let i = lastSH.index + 1; i <= currentIdx; i++) {
    if (candles[i].high > lastSH.price) {
      return false; // 直近スイングハイを超えた → 下降トレンド崩壊
    }
  }
  return true;
}
...
// メインシミュレーション
const allTrades = [];
const blockedByV2 = []; // v2条件でブロックされたケース
...
for (const day of days) {
  ...
  for (let i = 6; i < candles.length; i++) {
    ...
    // ④ 【v2追加】直近スイングハイ以降に高値更新なし
    if (!isStillInDowntrend(candles, swingHighs, i)) {
      blockedByV2.push({ day, time: candle.candleTime, price: candle.close, lastSH: swingHighs[swingHighs.length - 1] });
      continue;
    }
...
console.log("");
console.log("┌──────────────────────┬──────────────┬──────────────┬──────────────┬──────────────┐");
console.log("│ 指標                 │ Pullback Cfm │ Confirm Brk  │ 3山v1        │ 3山v2        │");
console.log("├──────────────────────┼──────────────┼──────────────┼──────────────┼──────────────┤");
console.log(`│ 取引数               │ 10件         │ 7件          │ 34件         │ ${String(allTrades.length).padStart(3)}件         │`);
console.log(`│ 勝率                 │ 20.0%        │ 42.9%        │ 32.4%        │ ${((wins.length / Math.max(allTrades.length, 1)) * 100).toFixed(1).padStart(5)}%       │`);
console.log(`│ PF                   │ 0.75         │ 2.25         │ 1.43         │ ${pf === Infinity ? "∞" : pf.toFixed(2).padStart(5)}        │`);
console.log(`│ 総損益               │ -10,081円    │ +24,838円    │ +49,576円    │ ${(totalPnl >= 0 ? "+" : "") + Math.round(totalPnl).toLocaleString().padStart(7)}円    │`);
console.log(`│ 最大DD               │ 24,912円     │ 9,975円      │ 29,876円     │ ${Math.round(maxDD).toLocaleString().padStart(7)}円    │`);
console.log(`│ 平均保有時間         │ 11.0分       │ 9.7分        │ 12.5分       │ ${avgHold.toFixed(1).padStart(5)}分      │`);
console.log(`│ 保有5分以内SL        │ 5件          │ 2件          │ 13件         │ ${String(sl5min).padStart(3)}件         │`);
console.log(`│ 期待値               │ -1,008円     │ +3,548円     │ +1,458円     │ ${(expectancy >= 0 ? "+" : "") + Math.round(expectancy).toLocaleString().padStart(6)}円      │`);
console.log("└──────────────────────┴──────────────┴──────────────┴──────────────┴──────────────┘");

// 目標判定
console.log("\n--- 目標達成判定 ---");
console.log(`取引数: ${allTrades.length}件 (v1: 34件から ${34 - allTrades.length}件削減)`);
...
// v2でブロックされた取引
console.log(`\n--- v2条件(下降トレンド継続確認)でブロックされた件数: ${blockedByV2.length}件 ---`);
for (const b of blockedByV2) {
  console.log(`  ${b.day} ${b.time} @${b.price} (直近SH: ${b.lastSH.time} ${b.lastSH.price} → その後に超える高値あり)`);
}
