...
import {
  ...
  type TaiyoCandidateBRejectionCode,
...
export interface SymbolConfig {
  ...
  advantestInitialShortWeakVolumeBlockMinRisePct?: number;
...
export const SYMBOL_CONFIG: Record<string, Partial<SymbolConfig>> = {
  ...
  "6976": {
    ...
    enableTrendLong: false,
    enableTrendShort: false,
    enableTaiyoCandidateB: true,
    enableTaiyoMorningInitialShort: false,
    ...
    enableTaiyoAfternoonReversalLong: false,
  ...
  "3436": {
    sl: { long: 0.5, short: 0.8 },
    tp: { long: 0.8, short: 0.7 },
  ...
  "6146": {
    ...
    // ポジションB: 本採用SHORTは一時停止。現行相当・A・Bは注文非接続の前向きシャドーで並行記録する。
    // LONG経路、他銘柄、データ受信には影響させない。
    enableDiscoOpeningBreakShort: false,
...
/** ★6976候補B: DRY_RUN実エントリー成功時だけ当日枠を消費し、拒否後は再探索する。 */
const taiyoCandidateBPrimaryFired = new Set<string>();
const taiyoCandidateBPending = new Map<string, TaiyoCandidateBPending>();
export interface TaiyoCandidateBAuditEvent {
  tradeDate: string;
  candleTime: string;
  symbol: string;
...
/** ★3436専用SHORT: DRY_RUN実エントリー成功時だけ当日枠を消費し、拒否後は再探索する。 */
const sumcoBreakdownShortFired = new Set<string>();
export interface SumcoBreakdownShortAuditEvent {
  tradeDate: string;
  candleTime: string;
  symbol: string;
...
async function recordSumcoBreakdownShortAuditEvent(
  ...
  sumcoBreakdownShortAuditEvents.push(event);
  if (event.event !== "engine_rejected" || event.referencePrice === undefined) return;
  if (typeof upsertSumcoBreakdownShortEvent !== "function") return;
  try {
    await upsertSumcoBreakdownShortEvent({
      tradeDate: event.tradeDate,
      symbol: event.symbol,
      candleTime: event.candleTime,
      eventType: event.event,
    ...
    console.error("[RealtimeSim] 3436専用SHORT監査イベントDB保存失敗:", error);
  }
}
...
function applySpecializedFiredState(symbol: string, key: SpecializedFiredStateKey): void {
  const target = {
    ...
    taiyoCandidateB: taiyoCandidateBPrimaryFired,
...
export function resolveRestoredRiskOverrides(
  ...
  if (symbol === "6976" && reason.startsWith("太陽誘電候補B")) {
    return {
      slPct: TAIYO_CANDIDATE_B_SPEC.primary.slPct,
      tpPct: TAIYO_CANDIDATE_B_SPEC.primary.tpPct,
    };
  }
  ...
  if (symbol === SUMCO_BREAKDOWN_SHORT_SPEC.symbol && reason.startsWith(SUMCO_BREAKDOWN_SHORT_REASON_PREFIX)) {
    return {
      slPct: SUMCO_BREAKDOWN_SHORT_SPEC.primary.slPct,
...
export function resolveSpecializedFiredStateKeys(
  ...
  if (symbol === "6976") {
    if ((action === "buy" || action === "short") && reason.startsWith("太陽誘電候補B")) return ["taiyoCandidateB"];
  ...
  if (
    symbol === SUMCO_BREAKDOWN_SHORT_SPEC.symbol &&
    action === "short" &&
...
export async function processCandle(candle: RtCandle1Min): Promise<{
  ...
  // ---- 3436前場15本安値更新SHORT: DRY_RUN正式経路 ----
  // 共通ゲート拒否では日次枠を消費せず、後続候補を再探索する。
  if (
    symConfig.enableSumcoBreakdownShort &&
    symbol === SUMCO_BREAKDOWN_SHORT_SPEC.symbol &&
    !sumcoBreakdownShortFired.has(symbol) &&
    isSumcoBreakdownShortEntryTime(candleTime)
  ) {
    const spec = SUMCO_BREAKDOWN_SHORT_SPEC.primary;
    const metrics = calculateSumcoBreakdownShortMetrics(buffer.map(item => ({
      time: item.time.slice(11, 16),
      open: item.open,
      high: item.high,
      low: item.low,
    ...
    if (metrics?.eligible) {
      await recordSumcoBreakdownShortAuditEvent({
        tradeDate,
        candleTime,
        symbol,
        event: "trigger",
      ...
      const result = await enterPosition(
        ...
        tradeDate,
        candleTime,
        `${SUMCO_BREAKDOWN_SHORT_REASON_PREFIX}: 15本終値安値更新、MA8二本傾き${metrics.maSlope2Pct.toFixed(3)}%、出来高${metrics.volumeRatio.toFixed(2)}倍`,
        boardSnapshot,
        { slPct: spec.slPct, tpPct: spec.tpPct },
      );
      await recordSumcoBreakdownShortAuditEvent({
      ...
      if (result.action === "entry") {
        ...
        signalHistory.unshift({
          ...
          symbol,
          symbolName: getStockName(symbol),
          action: "sumco_breakdown_short_block",
          price: candle.close,
          shares: 0,
          pnl: null,
  ...
  if (
    ...
    const candidateDayOpen = getTaiyoCandidateBDayOpen(candidateBuffer);
    const metrics = candidateDayOpen === null
      ? null
  ...
  if (
    ...
    symConfig.enableTaiyoCandidateB &&
    symbol === TAIYO_CANDIDATE_B_SPEC.symbol &&
    candleTime > TAIYO_CANDIDATE_B_SPEC.primary.confirmationEndTime
  ) {
    taiyoCandidateBPending.delete(symbol);
  }
  ...
  if (isEntryAllowed && buffer.length >= 21) {
    ...
    if (
      (symConfig.enableTaiyoMorningInitialShort || (taiyoCandidateAAuditEnabled && symbol === TAIYO_CANDIDATE_A_SPEC.symbol)) &&
  ...
  if (
    ...
    if (shortOk) {
      const weakVolumeBlocked = shouldBlockAdvantestInitialShortByWeakVolume({
        isReentry: isAdvantestShortReentry,
        riseFromOpenPct,
        volumeRatio,
        minRiseFromOpenPct: symConfig.advantestInitialShortWeakVolumeBlockMinRisePct ?? 1.9,
        maxVolumeRatioExclusive: symConfig.advantestInitialShortWeakVolumeBlockMaxVolumeRatio ?? 2.2,
      });
      if (weakVolumeBlocked) {
        console.log(
          `[RealtimeSim] ${symbol} アドバンテスト初回SHORT見送り: ` +
          `当日高値の始値比+${riseFromOpenPct.toFixed(2)}% >= ` +
          `${(symConfig.advantestInitialShortWeakVolumeBlockMinRisePct ?? 1.9).toFixed(1)}%、` +
          `初動出来高${volumeRatio.toFixed(2)}倍 < ` +
          `${(symConfig.advantestInitialShortWeakVolumeBlockMaxVolumeRatio ?? 2.2).toFixed(1)}倍（後続候補を再探索）`,
        );
      } else {
...
export async function enterPosition(
    ...
    if (latestATR !== null && price > 0) {
      ...
      if (atrRatio < ATR_FILTER_THRESHOLD) {
        ...
        return {
          ...
          candleTime,
          action: "none",
          reason: `atr_block:${(atrRatio * 100).toFixed(4)}%<${(ATR_FILTER_THRESHOLD * 100).toFixed(2)}%`,
        };
      }
    }
  }
  ...
  if (currentExposure + amount > MAX_TOTAL_EXPOSURE) {
    ...
    if (signalHistory.length > MAX_SIGNAL_HISTORY) signalHistory.length = MAX_SIGNAL_HISTORY;
    return { symbol, tradeDate, candleTime, action: "none", reason: "margin_block" };
  }
...
async function checkExitConditions(
  ...
  // 3436専用SHORT DRY_RUN: 30分境界の完成足終値を約定近似値として決済する。
  if (exitPrice === null && isSumcoBreakdownShortPosition) {
    const maxHoldingMinutes = SUMCO_BREAKDOWN_SHORT_SPEC.primary.maxHoldingMinutes;
    const elapsedMinutes = timeToMinutes(candleTime) - timeToMinutes(pos.entryTime);
    if (elapsedMinutes >= maxHoldingMinutes) {
      exitPrice = candle.close;
...
/** 6976候補Bの当日DRY_RUN監査イベントをVitestから取得する。 */
export function getTaiyoCandidateBAuditEventsForTest(): TaiyoCandidateBAuditEvent[] {
  if (process.env.VITEST !== "true") {
...
/** 3436専用SHORTの当日DRY_RUN監査イベントをVitestから取得する。 */
export function getSumcoBreakdownShortAuditEventsForTest(): SumcoBreakdownShortAuditEvent[] {
  if (process.env.VITEST !== "true") {
