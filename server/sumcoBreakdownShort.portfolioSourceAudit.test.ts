...
const BASELINE_SYMBOLS = ["285A", "8035", "5803", "6981", "6976", "6857", "6146", "6526"] as const;
const ALL_SYMBOLS = [...BASELINE_SYMBOLS, "3436"] as const;
...
const sourceAuditIt = process.env.RUN_SUMCO_PORTFOLIO_SOURCE_AUDIT === "1" ? it : it.skip;

describe("3436専用SHORT 9銘柄保存ID順ソース監査", () => {
  ...
  sourceAuditIt("現行8銘柄と3436追加9銘柄を比較し、統合期待値と資金競合を固定する", async () => {
    ...
    const baselineRows = allRows.filter(row => row.symbol !== "3436");

    expect(baselineRows).toHaveLength(108_151);
    expect(allRows).toHaveLength(117_568);

    const baseline = await replay(baselineRows);
    const candidate = await replay(allRows);
    const candidateExisting = candidate.trades.filter(trade => trade.symbol !== "3436");
    const sumcoTrades = candidate.trades.filter(trade => trade.symbol === "3436");

    expect(baseline.trades).toHaveLength(229);
    expect(baseline.trades.filter(trade => trade.pnl > 0)).toHaveLength(182);
    expect(baseline.trades.filter(trade => trade.pnl < 0)).toHaveLength(47);
    expect(baseline.trades.reduce((sum, trade) => sum + trade.pnl, 0)).toBe(4_281_533);

    expect(sumcoTrades).toHaveLength(22);
    expect(sumcoTrades.filter(trade => trade.pnl > 0)).toHaveLength(19);
    expect(sumcoTrades.filter(trade => trade.pnl < 0)).toHaveLength(3);
    expect(sumcoTrades.reduce((sum, trade) => sum + trade.pnl, 0)).toBe(277_637);
    expect(candidate.trades).toHaveLength(249);
    expect(candidate.trades.filter(trade => trade.pnl > 0)).toHaveLength(200);
    expect(candidate.trades.filter(trade => trade.pnl < 0)).toHaveLength(49);
    expect(candidate.trades.reduce((sum, trade) => sum + trade.pnl, 0)).toBe(4_589_426);

    expect(candidateExisting).toHaveLength(227);
    expect(candidateExisting.filter(trade => trade.pnl > 0)).toHaveLength(181);
    expect(candidateExisting.filter(trade => trade.pnl < 0)).toHaveLength(46);
    expect(candidateExisting.reduce((sum, trade) => sum + trade.pnl, 0)).toBe(4_311_789);
    expect(candidate.marginBlocks.filter(block => block.symbol === "3436").length).toBeGreaterThan(0);
  }, 300_000);
});
