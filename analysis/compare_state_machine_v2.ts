/**
 * compare_state_machine_v2.ts
 * 「戻り終了確認ステートマシン Ver2」比較バックテスト
 * 
 * Ver1からの変更点:
 * 【1】適用対象: 大台割れシグナルのみ（他は即エントリー維持）
 * 【2】待機: min=0, max=8本
 * 【3】戻り条件: 0.05%戻り or MA5接触 or VWAP接触
 * 【4】戻り終了: 陰線確定 + 終値<=MA5（板読み・buy_pressureはデータなしのためスキップ）
 * 【5】エントリー価格制御: entryPrice <= signalPrice 必須
 * 【6】追いかけ禁止: signalPriceから0.25%以上乖離でキャンセル
 */
...
// ステートマシン Ver2 定数
// ============================================================
const SM_MIN_WAIT = 0;            // 最低0本（即座に戻り条件チェック可）
const SM_MAX_WAIT = 8;            // 最大8本
const SM_PULLBACK_RATIO = 0.0005; // 0.05%戻り
const SM_MA5_TOUCH = true;        // MA5接触で戻り条件成立
const SM_VWAP_TOUCH = true;       // VWAP接触で戻り条件成立
const SM_MAX_DEVIATION = 0.0025;  // 0.25%以上乖離でキャンセル
...
// シグナル検出
// ============================================================
type SignalType = "round_number_break" | "vwap_cross_down" | "vwap_bearish_bounce" | "pullback_short" | "head_shoulders" | "other_short";
...
function evaluateSMv2(
  pending: PendingRoundBreak,
  candles: Candle[],
  currentBar: number
): "wait" | "enter" | "cancel" {
  const elapsed = currentBar - pending.signalBar;
  const curr = candles[currentBar];

  // 最大8本超過 → キャンセル
  if (elapsed > SM_MAX_WAIT) return "cancel";

  // 追いかけ禁止: signalPriceから0.25%以上下に乖離 → キャンセル
  const deviation = (pending.signalPrice - curr.close) / pending.signalPrice;
  if (deviation >= SM_MAX_DEVIATION) return "cancel";

  if (pending.state === "waiting_pullback") {
    // 戻り条件チェック
    const pullbackOk = checkPullbackV2(pending, candles, currentBar);
    if (pullbackOk) {
      pending.state = "waiting_reversal";
      // 同じ足で戻り終了も確認
      if (checkReversalV2(pending, candles, currentBar)) {
        // エントリー価格制御: entryPrice <= signalPrice
        if (curr.close <= pending.signalPrice) return "enter";
        // signalPriceより高い → まだ待つ
      }
    }
    return "wait";
  }
...
function checkPullbackV2(pending: PendingRoundBreak, candles: Candle[], i: number): boolean {
  const curr = candles[i];
  // ① 0.05%以上戻る（SHORTなので上に戻る）
  if (curr.close > pending.signalPrice * (1 + SM_PULLBACK_RATIO)) return true;
  // ② MA5接触
  if (SM_MA5_TOUCH && curr.ma5 && curr.high >= curr.ma5) return true;
  // ③ VWAP接触
  if (SM_VWAP_TOUCH && curr.vwap && curr.high >= curr.vwap) return true;
  return false;
}

function checkReversalV2(pending: PendingRoundBreak, candles: Candle[], i: number): boolean {
  const curr = candles[i];
  // 陰線確定
  if (curr.close >= curr.open) return false;
  // 終値がMA5以下
  if (!curr.ma5 || curr.close > curr.ma5) return false;
  // 板読みスコア>=1 → データなしのためスキップ（常にtrue扱い）
  // buy_pressureなし → データなしのためスキップ（常にtrue扱い）
  return true;
}
...
function simulateDay(
  candles: Candle[], symbol: string, date: string, useStateMachineV2: boolean
): { trades: TradeResult[]; smCancelled: number; smEntered: number } {
  ...
  let smCancelled = 0, smEntered = 0;

  // ステートマシン（大台割れ専用、1つだけ保持）
  let pendingSM: PendingRoundBreak | null = null;
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    if (shortShares > 0) {
      ...
      if (profitRate >= TAKE_PROFIT_RATIO) {
        ...
        shortShares = 0; shortEntryPrice = 0; shortLowWater = 0; shortEntryBar = -1; tradeCount++;
        continue;
      }
      // 損切り/トレイリング
    ...
    // ========== ステートマシン評価（ポジションなし時） ==========
    if (useStateMachineV2 && shortShares === 0 && pendingSM) {
      const result = evaluateSMv2(pendingSM, candles, i);
      if (result === "enter" && tradeCount < MAX_TRADES_PER_DAY && !isBlockedTime(curr.time)) {
        ...
        if (shares > 0) {
          ...
          shortEntryReason = pendingSM.reason + " [SM-v2]";
          shortSignalType = "round_number_break";
          smEntered++;
        }
        pendingSM = null;
    ...
    if (shortShares === 0 && tradeCount < MAX_TRADES_PER_DAY && !isBlockedTime(curr.time)) {
      ...
      for (const sig of signals) {
        ...
        if (useStateMachineV2 && sig.type === "round_number_break") {
          // 大台割れ → ステートマシンへ（既存pendingがあれば上書き）
          pendingSM = {
  ...
  return { trades, smCancelled, smEntered };
}

// 仮想取引（見送った場合の結果推定）
function simulateHypothetical(candles: Candle[], entryBar: number, entryPrice: number): number {
  ...
  for (let i = entryBar + 1; i < Math.min(entryBar + SHORT_MAX_HOLD_BARS, candles.length); i++) {
    ...
    if (c.close >= entryPrice * (1 + STOP_LOSS_RATIO)) return -1; // 損切り
    if ((entryPrice - c.close) / entryPrice >= TAKE_PROFIT_RATIO) return 1; // 利確
    const gain = (entryPrice - lowWater) / entryPrice;
    if (gain >= TRAIL_TRIGGER && c.close >= lowWater * (1 + TRAIL_GAP)) return 1; // トレイリング利確
  }
  const lastBar = Math.min(entryBar + SHORT_MAX_HOLD_BARS - 1, candles.length - 1);
  return candles[lastBar].close < entryPrice ? 1 : -1;
}
...
async function main() {
  ...
  const smv2All: TradeResult[] = [];
  let totalSmCancelled = 0, totalSmEntered = 0;
  ...
  for (const date of dates) {
    for (const symbol of Object.keys(grouped[date])) {
      ...
      currentAll.push(...curr.trades);

      const smv2 = simulateDay(candles.map(c => ({ ...c })), symbol, date, true);
      smv2All.push(...smv2.trades);
      totalSmCancelled += smv2.smCancelled;
      totalSmEntered += smv2.smEntered;
    }
  }
  ...
  const ss = agg(smv2All);

  // 大台割れ単体
  const currRound = currentAll.filter(t => t.signalType === "round_number_break");
  const smRound = smv2All.filter(t => t.signalType === "round_number_break");
  const currRoundPnl = currRound.reduce((s, t) => s + t.pnl, 0);
  const smRoundPnl = smRound.reduce((s, t) => s + t.pnl, 0);

  // 出力
  console.log("\n" + "=".repeat(70));
  console.log("  戻り終了確認ステートマシン Ver2 比較バックテスト結果");
  console.log("  適用対象: 大台割れシグナルのみ");
  ...
  console.log("\n【検証項目】");
  console.log(`${"指標".padEnd(22)}${"現行".padStart(14)}${"SM-v2".padStart(14)}${"差分".padStart(14)}`);
  console.log("-".repeat(64));
  const rows: [string, number, number][] = [
    ["総損益", cs.totalPnl, ss.totalPnl],
    ["Profit Factor", cs.pf, ss.pf],
    ["最大DD", cs.maxDD, ss.maxDD],
    ["損切り回数", cs.stopLoss, ss.stopLoss],
    ["大台割れ単体損益", currRoundPnl, smRoundPnl],
    ["エントリー数", cs.entryCount, ss.entryCount],
  ];
  ...
  // 追加指標
  console.log("\n【追加指標】");
  console.log(`  勝率: ${fmtPct(cs.winRate)}% → ${fmtPct(ss.winRate)}%`);
  console.log(`  平均利益: ${fmt(cs.avgWin)}円 → ${fmt(ss.avgWin)}円`);
  console.log(`  平均損失: ${fmt(cs.avgLoss)}円 → ${fmt(ss.avgLoss)}円`);
  console.log(`  利確回数: ${cs.takeProfit} → ${ss.takeProfit}`);
  console.log(`  SM発動→エントリー: ${totalSmEntered}件`);
  console.log(`  SM発動→キャンセル: ${totalSmCancelled}件`);
  ...
  console.log(`  SM-v2: ${smRound.length}件, ${smRound.filter(t => t.pnl > 0).length}W/${smRound.filter(t => t.pnl <= 0).length}L, P&L: ${fmt(smRoundPnl)}円`);

  // 合格基準
  console.log("\n【合格基準チェック】");
  const entryReduction = cs.entryCount > 0 ? (cs.entryCount - ss.entryCount) / cs.entryCount * 100 : 0;
  const criteria = [
    { name: "総損益改善", pass: ss.totalPnl > cs.totalPnl },
    { name: "ProfitFactor改善", pass: ss.pf > cs.pf },
    { name: "最大DD改善", pass: ss.maxDD < cs.maxDD },
    { name: "大台割れ損益プラス化", pass: smRoundPnl > 0 },
  ...
  // 日別
  console.log("\n【日別損益】");
  console.log(`${"日付".padEnd(12)}${"現行".padStart(12)}${"SM-v2".padStart(12)}${"差分".padStart(12)}`);
  for (const date of dates) {
    const cDay = currentAll.filter(t => t.date === date).reduce((s, t) => s + t.pnl, 0);
    const sDay = smv2All.filter(t => t.date === date).reduce((s, t) => s + t.pnl, 0);
    const diff = sDay - cDay;
    console.log(`${date.padEnd(12)}${fmt(cDay).padStart(12)}${fmt(sDay).padStart(12)}${((diff >= 0 ? "+" : "") + fmt(diff)).padStart(12)}`);
  }
