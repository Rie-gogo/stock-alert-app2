...
## A: デイリーストップ最適化 ＋ B: 動的資金配分（2026-06-03 着手）
- [ ] A: 停止ライン(-1万/-1.5万/-2万)×利益保護(なし/+1.5万/+2万/+3万)を上昇・下落両相場でスイープ
- [ ] A: 両相場でバランスの良い最適設定を決定し、本番デフォルト(DEFAULT_PORTFOLIO_CONFIG)に適用
- [ ] A: デイリーストップ設定の回帰テストを追加
- [ ] B: 勢い(出来高・値動き)ベースで調子の良い銘柄にロットを厚く配分するロジックを実装
- [ ] B: 資金配分ありで上昇・下落両相場をバックテストし改善前後を比較
- [ ] 型チェック0件・全テスト通過を確認
- [ ] チェックポイント保存・A/B成果の報告
