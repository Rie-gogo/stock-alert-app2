...
console.log("現行仕様シミュレーション（③改 medium直接禁止 + ⑤ 時間帯フィルター）");
console.log("（B・C適用版ベース、7日間）");
console.log("=".repeat(80));
console.log("");
...
const CONFIGS: SimConfig[] = [
  // ベースライン（改良前: medium直接OK + 時間帯フィルターなし）
  { label: "ベースライン（改良前: medium直接OK）", blockMediumDirect: false, timeFilter: false, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // 現行仕様（③改 medium直接禁止 + ⑤時間帯フィルター）
  { label: "現行仕様（③改+⑤: medium禁止+時間帯フィルター）", blockMediumDirect: true, timeFilter: true, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
];
...
console.log("=".repeat(80));
console.log("");
console.log("日付       | ベースライン(改良前)  | 現行仕様(③改+⑤)  | 差分");
console.log("-----------|-------------------|-------------------|----------");
for (let i = 0; i < DATES.length; i++) {
  const d = DATES[i];
  const base = results[0].daily[i];
  const curr = results[1].daily[i];
  const diff = curr.pnl - base.pnl;
  console.log(`${d} | ${(base.pnl >= 0 ? "+" : "") + Math.round(base.pnl).toLocaleString().padStart(10)}円(${base.trades}T) | ${(curr.pnl >= 0 ? "+" : "") + Math.round(curr.pnl).toLocaleString().padStart(10)}円(${curr.trades}T) | ${(diff >= 0 ? "+" : "") + Math.round(diff).toLocaleString()}円`);
}

// トレード詳細
console.log("");
console.log("=".repeat(80));
console.log("現行仕様 全トレード詳細");
console.log("=".repeat(80));
const currentConfig = CONFIGS[1];
for (const date of DATES) {
  const dayData = allDayData.get(date)!;
  if (dayData.length === 0) continue;
  const result = simulateDay(dayData, currentConfig);
  if (result.trades.length === 0) continue;
  console.log(`\n--- ${date} ---`);
  for (const t of result.trades) {
    if (t.pnl !== undefined) {
      console.log(`  ${t.time} ${t.sym} ${t.action} @${t.price} ${t.reason} P&L:${t.pnl >= 0 ? "+" : ""}${Math.round(t.pnl).toLocaleString()}円 [${t.confidence || "-"}]`);
    } else {
      console.log(`  ${t.time} ${t.sym} ${t.action} @${t.price} ${t.reason} [${t.confidence || "-"}]`);
    }
  }
}
