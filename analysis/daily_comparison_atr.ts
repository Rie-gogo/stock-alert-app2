/**
 * daily_comparison_atr.ts
 * ベースライン（固定SL0.5/TP1.5）と最適パラメータ（ATR×3.0 SL[0.3-1.0] RR2.0）の日別比較
 * sweep_atr_final.tsのsimulateDay関数を再利用
 */
...
interface AtrSlParams { atrPeriod: number; atrMultiplier: number; minSl: number; maxSl: number; rrRatio: number; fixedTp: number; label: string; }
...
function calcDynamicSlTp(buffer: CandleWithSignal[], close: number, params: AtrSlParams): { slPct: number; tpPct: number } {
  if (params.atrMultiplier === 0) return { slPct: params.minSl, tpPct: params.fixedTp };
  if (buffer.length < params.atrPeriod + 1) { const sl = Math.max(params.minSl, Math.min(params.maxSl, 0.5)); const tp = params.rrRatio > 0 ? sl * params.rrRatio : params.fixedTp; return { slPct: sl, tpPct: tp }; }
  const h = buffer.map(c => c.high); const l = buffer.map(c => c.low); const cl = buffer.map(c => c.close);
  const atr = calcATR(h, l, cl, params.atrPeriod); const latestATR = atr[atr.length - 1];
  if (latestATR === null || close <= 0) { const sl = Math.max(params.minSl, Math.min(params.maxSl, 0.5)); const tp = params.rrRatio > 0 ? sl * params.rrRatio : params.fixedTp; return { slPct: sl, tpPct: tp }; }
...
function simulateDay(dayData: RtCandleRow[], params: AtrSlParams): { trades: Trade[]; totalPnl: number } {
  ...
  const sorted = dayData.filter(c => ALLOWED_SYMBOLS.has(c.symbol)).filter(c => !(c.candleTime >= "11:30" && c.candleTime < "12:30")).sort((a, b) => a.candleTime < b.candleTime ? -1 : a.candleTime > b.candleTime ? 1 : a.symbol < b.symbol ? -1 : 1);
  ...
  for (const candle of sorted) {
    ...
    if (existingPos) {
      ...
      if (candleTime >= MARKET_CLOSE_TIME) { const pnl = side === "long" ? (close - entryPrice) * shares : (entryPrice - close) * shares; totalPnl += pnl; trades.push({ date: tradeDate, time: candleTime, sym: symbol, action: "forced_close", price: close, pnl, reason: "大引け強制決済", shares }); openPositions.delete(symbol); continue; }
      let exitPrice: number | null = null, exitReason = "";
      if (side === "long") { const sl = entryPrice * (1 - slPct / 100); const tp = entryPrice * (1 + tpPct / 100); if (low <= sl) { exitPrice = sl; exitReason = "損切り"; } else if (high >= tp) { exitPrice = tp; exitReason = "利確"; } }
      else { const sl = entryPrice * (1 + slPct / 100); const tp = entryPrice * (1 - tpPct / 100); if (high >= sl) { exitPrice = sl; exitReason = "損切り"; } else if (low <= tp) { exitPrice = tp; exitReason = "利確"; } }
    ...
    if (ps) { ps.waitCount++; if (low < ps.recentSwingLow || ps.waitCount > PULLBACK_MAX_WAIT) { pullbackStates.delete(symbol); } else { if (!ps.pulledBack && close < ps.signalPrice) ps.pulledBack = true; if (ps.pulledBack && close > ps.signalPrice) { pullbackStates.delete(symbol); if (boardSnapshot?.signal === "sell_pressure") { continue; } const brScore = getBoardScore("long"); if (brScore < BOARD_SCORE_THRESHOLD) { continue; } const shares = calcShares(close); const amount = close * shares; if (buffer.length >= ATR_FILTER_PERIOD + 1) { const h2 = buffer.map(c => c.high); const l2 = buffer.map(c => c.low); const cl2 = buffer.map(c => c.close); const atr2 = calcATR(h2, l2, cl2, ATR_FILTER_PERIOD); if (atr2[atr2.length-1] !== null && close > 0 && (atr2[atr2.length-1]! / close) < ATR_FILTER_THRESHOLD) { continue; } } if (calcCurrentExposure(openPositions) + amount > MAX_TOTAL_EXPOSURE) { continue; } if (checkVolumeUnavailable(buffer)) { continue; } const { slPct, tpPct } = calcDynamicSlTp(buffer, close, params); openPositions.set(symbol, { symbol, side: "long", entryPrice: close, shares, entryTime: candleTime, entryReason: ps.reason, slPct, tpPct }); trades.push({ date: tradeDate, time: candleTime, sym: symbol, action: "buy", price: close, reason: ps.reason, shares }); } } continue; }

    const rp = roundLevelPendingStates.get(symbol);
    if (rp) { if (openPositions.has(symbol)) { roundLevelPendingStates.delete(symbol); } else { const valid = rp.direction === "buy" ? close >= rp.level : close <= rp.level; if (valid) { rp.confirmCount++; if (rp.confirmCount >= ROUND_LEVEL_CONFIRM_BARS) { roundLevelPendingStates.delete(symbol); roundPullbackStates.set(symbol, { ...rp, signalPrice: close, waitCount: 0, pulledBack: false }); } } else { roundLevelPendingStates.delete(symbol); } continue; } }
    ...
    if (rpb) {
      ...
      if (rpb.waitCount > ROUND_PULLBACK_MAX_WAIT) { roundPullbackStates.delete(symbol); if (boardSnapshot && side === "long" && boardSnapshot.signal === "sell_pressure") { continue; } if (boardSnapshot && side === "short" && boardSnapshot.signal === "buy_pressure") { continue; } const brScore = getBoardScore(side); if (brScore < BOARD_SCORE_THRESHOLD) { continue; } const shares = calcShares(close); const amount = close * shares; if (buffer.length >= ATR_FILTER_PERIOD + 1) { const h2 = buffer.map(c => c.high); const l2 = buffer.map(c => c.low); const cl2 = buffer.map(c => c.close); const atr2 = calcATR(h2, l2, cl2, ATR_FILTER_PERIOD); if (atr2[atr2.length-1] !== null && close > 0 && (atr2[atr2.length-1]! / close) < ATR_FILTER_THRESHOLD) { continue; } } if (calcCurrentExposure(openPositions) + amount > MAX_TOTAL_EXPOSURE) { continue; } const { slPct, tpPct } = calcDynamicSlTp(buffer, close, params); openPositions.set(symbol, { symbol, side, entryPrice: close, shares, entryTime: candleTime, entryReason: `${rpb.reason} (押し目なし)`, slPct, tpPct }); trades.push({ date: tradeDate, time: candleTime, sym: symbol, action: side === "long" ? "buy" : "short", price: close, reason: `${rpb.reason} (押し目なし)`, shares }); continue; }
      if (rpb.direction === "buy") { if (!rpb.pulledBack && close < rpb.signalPrice) rpb.pulledBack = true; if (rpb.pulledBack && close > rpb.signalPrice) { roundPullbackStates.delete(symbol); if (boardSnapshot?.signal === "sell_pressure") continue; const brScore = getBoardScore("long"); if (brScore < BOARD_SCORE_THRESHOLD) continue; const shares = calcShares(close); const amount = close * shares; if (buffer.length >= ATR_FILTER_PERIOD + 1) { const h2 = buffer.map(c => c.high); const l2 = buffer.map(c => c.low); const cl2 = buffer.map(c => c.close); const atr2 = calcATR(h2, l2, cl2, ATR_FILTER_PERIOD); if (atr2[atr2.length-1] !== null && close > 0 && (atr2[atr2.length-1]! / close) < ATR_FILTER_THRESHOLD) continue; } if (calcCurrentExposure(openPositions) + amount > MAX_TOTAL_EXPOSURE) continue; const { slPct, tpPct } = calcDynamicSlTp(buffer, close, params); openPositions.set(symbol, { symbol, side: "long", entryPrice: close, shares, entryTime: candleTime, entryReason: `${rpb.reason} (押し目確認後)`, slPct, tpPct }); trades.push({ date: tradeDate, time: candleTime, sym: symbol, action: "buy", price: close, reason: `${rpb.reason} (押し目確認後)`, shares }); continue; } }
      else { if (!rpb.pulledBack && close > rpb.signalPrice) rpb.pulledBack = true; if (rpb.pulledBack && close < rpb.signalPrice) { roundPullbackStates.delete(symbol); if (boardSnapshot?.signal === "buy_pressure") continue; const brScore = getBoardScore("short"); if (brScore < BOARD_SCORE_THRESHOLD) continue; const shares = calcShares(close); const amount = close * shares; if (buffer.length >= ATR_FILTER_PERIOD + 1) { const h2 = buffer.map(c => c.high); const l2 = buffer.map(c => c.low); const cl2 = buffer.map(c => c.close); const atr2 = calcATR(h2, l2, cl2, ATR_FILTER_PERIOD); if (atr2[atr2.length-1] !== null && close > 0 && (atr2[atr2.length-1]! / close) < ATR_FILTER_THRESHOLD) continue; } if (calcCurrentExposure(openPositions) + amount > MAX_TOTAL_EXPOSURE) continue; const { slPct, tpPct } = calcDynamicSlTp(buffer, close, params); openPositions.set(symbol, { symbol, side: "short", entryPrice: close, shares, entryTime: candleTime, entryReason: `${rpb.reason} (押し目確認後)`, slPct, tpPct }); trades.push({ date: tradeDate, time: candleTime, sym: symbol, action: "short", price: close, reason: `${rpb.reason} (押し目確認後)`, shares }); continue; } }
      continue;
    }

    const sig = latestSignal.signal; if (!sig) continue;
    if (sig.type === "buy" && !openPositions.has(symbol)) {
      ...
      else { const shares = calcShares(close); const amount = close * shares; if (buffer.length >= ATR_FILTER_PERIOD + 1) { const h2 = buffer.map(c => c.high); const l2 = buffer.map(c => c.low); const cl2 = buffer.map(c => c.close); const atr2 = calcATR(h2, l2, cl2, ATR_FILTER_PERIOD); if (atr2[atr2.length-1] !== null && close > 0 && (atr2[atr2.length-1]! / close) < ATR_FILTER_THRESHOLD) continue; } if (calcCurrentExposure(openPositions) + amount > MAX_TOTAL_EXPOSURE) continue; if (checkVolumeUnavailable(buffer)) continue; const lastSL = lastStopLossTime.get(symbol); if (lastSL && timeToMinutes(candleTime) - timeToMinutes(lastSL) < NO_REENTRY_AFTER_STOPLOSS_MIN) continue; const { slPct, tpPct } = calcDynamicSlTp(buffer, close, params); openPositions.set(symbol, { symbol, side: "long", entryPrice: close, shares, entryTime: candleTime, entryReason: sig.reason, slPct, tpPct }); trades.push({ date: tradeDate, time: candleTime, sym: symbol, action: "buy", price: close, reason: sig.reason!, shares }); }
    }
    if (sig.type === "sell" && !openPositions.has(symbol)) {
      ...
      const shares = calcShares(close); const amount = close * shares; if (buffer.length >= ATR_FILTER_PERIOD + 1) { const h2 = buffer.map(c => c.high); const l2 = buffer.map(c => c.low); const cl2 = buffer.map(c => c.close); const atr2 = calcATR(h2, l2, cl2, ATR_FILTER_PERIOD); if (atr2[atr2.length-1] !== null && close > 0 && (atr2[atr2.length-1]! / close) < ATR_FILTER_THRESHOLD) continue; } if (calcCurrentExposure(openPositions) + amount > MAX_TOTAL_EXPOSURE) continue; if (checkVolumeUnavailable(buffer)) continue; const lastSL = lastStopLossTime.get(symbol); if (lastSL && timeToMinutes(candleTime) - timeToMinutes(lastSL) < NO_REENTRY_AFTER_STOPLOSS_MIN) continue; const { slPct, tpPct } = calcDynamicSlTp(buffer, close, params); openPositions.set(symbol, { symbol, side: "short", entryPrice: close, shares, entryTime: candleTime, entryReason: sig.reason, slPct, tpPct }); trades.push({ date: tradeDate, time: candleTime, sym: symbol, action: "short", price: close, reason: sig.reason!, shares }); }
  }

  for (const [symbol, pos] of openPositions.entries()) { const symCandles = sorted.filter(c => c.symbol === symbol); const last = symCandles[symCandles.length - 1]; if (!last) continue; const pnl = pos.side === "long" ? (last.close - pos.entryPrice) * pos.shares : (pos.entryPrice - last.close) * pos.shares; totalPnl += pnl; trades.push({ date: last.tradeDate, time: last.candleTime, sym: symbol, action: "forced_close", price: last.close, pnl, reason: "データ終了時決済", shares: pos.shares }); }
...
const BASELINE: AtrSlParams = { atrPeriod: 30, atrMultiplier: 0, minSl: 0.5, maxSl: 0.5, rrRatio: 0, fixedTp: 1.5, label: "固定 SL0.5/TP1.5" };
const OPTIMAL: AtrSlParams = { atrPeriod: 30, atrMultiplier: 3.0, minSl: 0.3, maxSl: 1.0, rrRatio: 2.0, fixedTp: 0, label: "ATR×3.0 SL[0.3-1.0] RR2.0" };

console.log("=".repeat(90));
console.log("日別損益比較: ベースライン vs 最適パラメータ（ATR×3.0 SL[0.3-1.0] RR2.0）");
console.log("=".repeat(90));
console.log("");
console.log("日付        | ベースライン（固定SL/TP）      | ATR連動（最適）              | 差分");
console.log("------------|-------------------------------|-------------------------------|----------");

let totalBase = 0, totalOpt = 0;
let totalTradesBase = 0, totalTradesOpt = 0;
let totalWinsBase = 0, totalWinsOpt = 0;
let totalSLBase = 0, totalSLOpt = 0;

for (const date of DATES) {
  const dayData = loadDayData(date);
  if (dayData.length === 0) { console.log(`${date}  | データなし`); continue; }
  
  const baseResult = simulateDay(dayData, BASELINE);
  const optResult = simulateDay(dayData, OPTIMAL);
  
  const baseClosed = baseResult.trades.filter(t => t.pnl !== undefined);
  const optClosed = optResult.trades.filter(t => t.pnl !== undefined);
  const baseWins = baseClosed.filter(t => (t.pnl ?? 0) > 0).length;
  const optWins = optClosed.filter(t => (t.pnl ?? 0) > 0).length;
  const baseSL = baseClosed.filter(t => t.reason === "損切り").length;
  const optSL = optClosed.filter(t => t.reason === "損切り").length;
  
  totalBase += baseResult.totalPnl; totalOpt += optResult.totalPnl;
  totalTradesBase += baseClosed.length; totalTradesOpt += optClosed.length;
  totalWinsBase += baseWins; totalWinsOpt += optWins;
  totalSLBase += baseSL; totalSLOpt += optSL;
  
  const diff = optResult.totalPnl - baseResult.totalPnl;
  const baseStr = `${(baseResult.totalPnl >= 0 ? "+" : "")}${Math.round(baseResult.totalPnl).toLocaleString()}円 (${baseClosed.length}T W${baseWins} SL${baseSL})`;
  const optStr = `${(optResult.totalPnl >= 0 ? "+" : "")}${Math.round(optResult.totalPnl).toLocaleString()}円 (${optClosed.length}T W${optWins} SL${optSL})`;
  const diffStr = `${(diff >= 0 ? "+" : "")}${Math.round(diff).toLocaleString()}円`;
  console.log(`${date}  | ${baseStr.padEnd(28)} | ${optStr.padEnd(28)} | ${diffStr}`);
}

console.log("------------|-------------------------------|-------------------------------|----------");
const totalDiff = totalOpt - totalBase;
console.log(`合計        | ${((totalBase >= 0 ? "+" : "") + Math.round(totalBase).toLocaleString() + "円 (" + totalTradesBase + "T W" + totalWinsBase + " SL" + totalSLBase + ")").padEnd(28)} | ${((totalOpt >= 0 ? "+" : "") + Math.round(totalOpt).toLocaleString() + "円 (" + totalTradesOpt + "T W" + totalWinsOpt + " SL" + totalSLOpt + ")").padEnd(28)} | ${(totalDiff >= 0 ? "+" : "") + Math.round(totalDiff).toLocaleString()}円`);
console.log("");
console.log(`ベースライン勝率: ${Math.round(totalWinsBase/totalTradesBase*100)}% (${totalWinsBase}/${totalTradesBase})`);
console.log(`ATR連動勝率:     ${Math.round(totalWinsOpt/totalTradesOpt*100)}% (${totalWinsOpt}/${totalTradesOpt})`);
