...
import { FORWARD_EVALUATION_POLICY, TEL_EXECUTABLE_DEPTH_VERSION } from "./runtimeIdentity";
export { TEL_EXECUTABLE_DEPTH_VERSION } from "./runtimeIdentity";
import {
...
export interface TelExecutablePosition {
  ...
  executionProxyKind: "ask_depth_vwap" | "bid_depth_vwap";
...
function normalizedDepth(value: unknown, key: "asks" | "bids", side: "long" | "short"): DepthLevel[] {
  if (!value || typeof value !== "object") return [];
  const raw = (value as Record<string, unknown>)[key];
  if (!Array.isArray(raw)) return [];
  return raw
    .map(level => ({
      price: Number((level as { price?: unknown }).price),
      qty: Number((level as { qty?: unknown }).qty),
    }))
...
export function calculateDepthVwap(input: {
  board: unknown;
  side: "long" | "short";
  shares: number;
}): { price: number; filledShares: number; availableShares: number; levelsUsed: number } | null {
  const levels = normalizedDepth(input.board, input.side === "long" ? "asks" : "bids", input.side);
  const availableShares = levels.reduce((sum, level) => sum + level.qty, 0);
  if (availableShares < input.shares) return null;
...
export function applyTelExecutableConfirmTransition(stateBefore: TelExecutableConfirmState, input: ForwardSourceEventInput, mode: ForwardEvaluationMode): TelExecutableTransition {
  ...
  if (state.pending && !state.position && !state.dailySlotConsumed) {
    ...
    const levels = normalizedDepth(input.board, pending.side === "long" ? "asks" : "bids", pending.side);
    const bestPrice = levels[0]?.price ?? null;
    const shares = bestPrice === null ? null : sharesFor(mode, bestPrice);
    const depth = shares === null ? null : calculateDepthVwap({ board: input.board, side: pending.side, shares });
    const executable = depth?.price ?? null;
    ...
    const breakoutMaintained = executable !== null && (pending.side === "long"
      ...
      : executable < pending.breakoutLevel);
    const adverseAllowed = adversePct !== null && adversePct <= TEL_EXECUTABLE_DEPTH_MAX_ADVERSE_PCT;
    if (!timestampsAvailable || !causal || !fresh || executable === null || !breakoutMaintained || !adverseAllowed) {
      resultType = "rejected";
      actions.push({
        type: "entry_rejected",
        reason: !timestampsAvailable
          ...
          : !causal
            ...
            : !fresh
              ...
              : executable === null
                ? "insufficient_orderbook_depth_for_shares"
                : !breakoutMaintained
        ...
        executablePriceProxyKind: pending.side === "long" ? "ask_depth_vwap" : "bid_depth_vwap",
        shares,
        depth,
        boardObservedAtMs,
      ...
      state.position = {
        ...
        breakoutLevel: pending.breakoutLevel,
        adverseEntryPct: adversePct!,
        executionProxyKind: pending.side === "long" ? "ask_depth_vwap" : "bid_depth_vwap",
        boardObservedAtMs: boardObservedAtMs!,
      ...
      actions.push({
        ...
        breakoutLevel: pending.breakoutLevel,
        executablePriceProxy: executable,
        executablePriceProxyKind: state.position.executionProxyKind,
        depth,
        boardObservedAtMs,
        boardSourcePriceTimeMs,
        decisionAtMs,
...
export const TEL_EXECUTABLE_DEPTH_SPEC = Object.freeze({
  ...
  entry: Object.freeze({
    ...
    long: "ask_depth_vwap_for_shares > original_5bar_high",
