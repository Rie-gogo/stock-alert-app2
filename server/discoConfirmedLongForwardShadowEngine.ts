...
import {
  ...
  DISCO_LONG_PRIOR_THREE_B_SPEC,
  DISCO_LONG_PROFIT_PROTECTION_A_SPEC,
  applyDiscoLongPriorThreeBTransition,
  applyDiscoLongProfitProtectionATransition,
  createEmptyDiscoLongState,
  normalizeDiscoLongState,
  type DiscoLongState,
  type DiscoLongTransition,
} from "./discoConfirmedLongForwardShadow";

const MODES: readonly ForwardEvaluationMode[] = FORWARD_EVALUATION_POLICY.evaluationModes;
type Variant = DiscoLongState["variant"];

const DEFINITIONS = {
  profit_protection_a: {
    strategyVersion: DISCO_LONG_PROFIT_PROTECTION_A_VERSION,
    strategyId: "candidate-6146-confirmed-long-profit-protection-a",
    spec: DISCO_LONG_PROFIT_PROTECTION_A_SPEC,
    transition: applyDiscoLongProfitProtectionATransition,
    statusReason: "formal_evaluation_gate_pending_and_disco_confirmed_long_route_parity_required",
  },
  prior_three_b: {
    strategyVersion: DISCO_LONG_PRIOR_THREE_B_VERSION,
    strategyId: "candidate-6146-confirmed-long-prior-three-b",
    spec: DISCO_LONG_PRIOR_THREE_B_SPEC,
    transition: applyDiscoLongPriorThreeBTransition,
    statusReason: "formal_evaluation_gate_pending_and_disco_confirmed_long_route_parity_required",
  },
} as const;
...
async function ensureVersion(variant: Variant) {
  ...
  const config = {
    ...
    formalEvaluationStartDate: DISCO_LONG_FORWARD_FORMAL_START_DATE,
  ...
  await upsertRtStrategyVersion({
    ...
    configJson: config,
    learningCutoffDate: DISCO_LONG_FORWARD_LEARNING_CUTOFF_DATE,
...
async function acquireWithWait(variant: Variant, sourceEventId: string, mode: ForwardEvaluationMode) {
  ...
  throw new Error(`disco_long_forward_state_lock_timeout:${variant}:${mode}`);
}
...
async function persistTransition(input: {
  ...
  transition: DiscoLongTransition;
...
async function processMode(source: ForwardSourceEventInput, variant: Variant, mode: ForwardEvaluationMode) {
  ...
  const normalized = normalizeDiscoLongState(initialState?.stateJson, variant, source.candle.tradeDate);
  ...
  try {
    ...
    const state = normalizeDiscoLongState(latest?.stateJson, variant, source.candle.tradeDate);
...
export async function processDiscoConfirmedLongForwardShadowSourceEvent(source: ForwardSourceEventInput) {
  if (source.candle.symbol !== "6146") return { skipped: "non_6146_symbol" as const };
  if (source.candle.tradeDate < DISCO_LONG_FORWARD_COLLECTION_START_DATE) {
  ...
  const errors: string[] = [];
  for (const variant of ["profit_protection_a", "prior_three_b"] as const) {
    try {
  ...
  if (errors.length > 0) throw new Error(`disco_long_forward_shadow_partial_failure:${errors.join(" | ")}`);
  return { skipped: false as const, symbol: "6146", evaluations };
}

export function replayDiscoConfirmedLongForwardShadowDay(
...
export function auditDiscoConfirmedLongForwardShadowDay(
  ...
  for (const mode of MODES) {
    ...
    for (const sourceEvent of sourceEvents) {
      ...
      state = normalizeDiscoLongState(state, variant, source.candle.tradeDate);
      const stateHashBefore = sha256Stable(state);
...
export function resetDiscoConfirmedLongForwardVersionCacheForTest() {
