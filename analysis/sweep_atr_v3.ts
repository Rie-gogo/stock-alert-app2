/**
 * sweep_atr_v3.ts - 高速版ATRスイープ
 * パターン数を大幅に削減し、最も有望な組み合わせのみテスト
 * 
 * 戦略:
 * 1. モードA（SLのみATR連動、TP固定1.5%）: 15パターン
 * 2. モードB（両方ATR連動）: 15パターン
 * 合計30パターン + ベースライン
 */
...
function boardReadingScoreBC(bprHistoryArr: number[], side: "long" | "short", snapshot: BoardSnapshot | null): number {
  ...
  let mode: string;
  if (cancelDetected) { mode = "trap"; } else {
    if (bprHistoryArr.length >= 3 && bprHistoryArr.every(h => h >= 0.85 && h <= 1.15) && bpr >= 0.85 && bpr <= 1.15) mode = "quiet";
    else if (bpr > 1.2 || bpr < 0.8) mode = "active";
    else if (bprHistoryArr.length >= 3 && Math.abs(bprHistoryArr[bprHistoryArr.length - 1] - bprHistoryArr[0]) >= 0.1) mode = "building";
    else mode = "trap";
  }
  ...
  if (mod === "buy") tickDir = "uptick"; else if (mod === "sell") tickDir = "downtick";
  else if (bprHistoryArr.length >= 3) { const trend = bprHistoryArr[bprHistoryArr.length-1] - bprHistoryArr[0]; if (trend >= 0.2) tickDir = "uptick"; else if (trend <= -0.2) tickDir = "downtick"; else if (bprHistoryArr[bprHistoryArr.length-1] >= 1.3) tickDir = "uptick"; else if (bprHistoryArr[bprHistoryArr.length-1] <= 0.7) tickDir = "downtick"; }
  if (tickDir === "uptick") { if (side === "long") score += 2; else score -= 2; }
  else if (tickDir === "downtick") { if (side === "short") score += 2; else score -= 2; }
  if (snap.icebergAskDetected) { if (side === "long") score += 1; else score -= 1; }
  if (snap.icebergBidDetected) { if (side === "short") score += 1; else score -= 1; }
  return score;
}
...
function getSlTp(buffer: CandleWithSignal[], price: number, params: SweepParams): { slPct: number; tpPct: number } {
  ...
  const rawSlPct = (latestATR / price) * params.atrMultiplier * 100;
  const slPct = Math.max(params.minSl, Math.min(params.maxSl, rawSlPct));
  const tpPct = params.mode === "A" ? 1.5 : slPct * params.rrRatio;
  return { slPct, tpPct };
}

function simulateDay(dayData: RtCandleRow[], params: SweepParams): { totalPnl: number; trades: number; wins: number; losses: number; stopLosses: number; slPcts: number[] } {
  ...
  for (const candle of sorted) {
    ...
    if (existingPos) {
      ...
      if (candleTime >= MARKET_CLOSE_TIME) { const pnl = side === "long" ? (close - entryPrice) * shares : (entryPrice - close) * shares; totalPnl += pnl; if (pnl > 0) wins++; else losses++; openPositions.delete(symbol); continue; }
      ...
      if (exitPrice !== null) { const pnl = side === "long" ? (exitPrice - entryPrice) * shares : (entryPrice - exitPrice) * shares; totalPnl += pnl; if (pnl > 0) wins++; else losses++; if (exitReason === "損切り") { stopLosses++; lastStopLossTime.set(symbol, candleTime); } openPositions.delete(symbol); }
      continue;
    }
    ...
    const doEntry = (side: "long" | "short", reason: string): boolean => {
      ...
      const { slPct, tpPct } = getSlTp(buffer, close, params);
      slPcts.push(slPct);
      openPositions.set(symbol, { symbol, side, entryPrice: close, shares, entryTime: candleTime, entryReason: reason, slPct, tpPct });
      tradeCount++;
      return true;
    };
    ...
    if (ps) {
      ...
      else { if (!ps.pulledBack && close < ps.signalPrice) ps.pulledBack = true; if (ps.pulledBack && close > ps.signalPrice) { pullbackStates.delete(symbol); if (boardSnapshot?.signal !== "sell_pressure") doEntry("long", ps.reason); } }
      continue;
    }
    ...
    if (rpb) {
      ...
      if (rpb.waitCount > ROUND_PULLBACK_MAX_WAIT) { roundPullbackStates.delete(symbol); if (boardSnapshot && side === "long" && boardSnapshot.signal === "sell_pressure") { continue; } if (boardSnapshot && side === "short" && boardSnapshot.signal === "buy_pressure") { continue; } doEntry(side, `${rpb.reason} (押し目なし)`); continue; }
      if (rpb.direction === "buy") { if (!rpb.pulledBack && close < rpb.signalPrice) rpb.pulledBack = true; if (rpb.pulledBack && close > rpb.signalPrice) { roundPullbackStates.delete(symbol); if (boardSnapshot?.signal !== "sell_pressure") doEntry("long", `${rpb.reason} (押し目確認後)`); continue; } }
      else { if (!rpb.pulledBack && close > rpb.signalPrice) rpb.pulledBack = true; if (rpb.pulledBack && close < rpb.signalPrice) { roundPullbackStates.delete(symbol); if (boardSnapshot?.signal !== "buy_pressure") doEntry("short", `${rpb.reason} (押し目確認後)`); continue; } }
      continue;
    }
    ...
    if (sig.type === "sell" && !openPositions.has(symbol)) {
      ...
      doEntry("short", sig.reason!);
    }
  }

  for (const [symbol, pos] of openPositions.entries()) { const symCandles = sorted.filter(c => c.symbol === symbol); const last = symCandles[symCandles.length - 1]; if (!last) continue; const pnl = pos.side === "long" ? (last.close - pos.entryPrice) * pos.shares : (pos.entryPrice - last.close) * pos.shares; totalPnl += pnl; if (pnl > 0) wins++; else losses++; }
  return { totalPnl, trades: tradeCount, wins, losses, stopLosses, slPcts };
}
...
console.log(`データ: ${allDayData.length}日分ロード完了`);

// ベースライン
console.log("\n--- ベースライン（固定 SL=0.5% TP=1.5%） ---");
const fixedP: SweepParams = { mode: "fixed", atrPeriod: 30, atrMultiplier: 3, minSl: 0.5, maxSl: 0.5, rrRatio: 3 };
let basePnl = 0, baseTrades = 0, baseWins = 0, baseLosses = 0, baseSL = 0;
for (const { date, data } of allDayData) {
  const r = simulateDay(data, fixedP);
  basePnl += r.totalPnl; baseTrades += r.trades; baseWins += r.wins; baseLosses += r.losses; baseSL += r.stopLosses;
  console.log(`  ${date}: ${r.totalPnl >= 0 ? "+" : ""}${Math.round(r.totalPnl).toLocaleString()}円 | ${r.trades}T | W${r.wins} L${r.losses} | SL${r.stopLosses}`);
}
console.log(`  合計: ${basePnl >= 0 ? "+" : ""}${Math.round(basePnl).toLocaleString()}円 | ${baseTrades}T | 勝率${((baseWins/(baseWins+baseLosses))*100).toFixed(1)}% | SL${baseSL}`);

// スイープ（厳選パラメータ）
const sweepConfigs: SweepParams[] = [
  // モードA: SLのみATR連動、TP固定1.5%
  { mode: "A", atrPeriod: 30, atrMultiplier: 3.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 4.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 6.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 4.0, minSl: 0.3, maxSl: 1.0, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.3, maxSl: 1.0, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 6.0, minSl: 0.3, maxSl: 1.0, rrRatio: 3 },
  { mode: "A", atrPeriod: 50, atrMultiplier: 3.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 50, atrMultiplier: 4.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 50, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 20, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 20, atrMultiplier: 6.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 4.0, minSl: 0.4, maxSl: 0.7, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.4, maxSl: 0.7, rrRatio: 3 },
  { mode: "A", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.5, maxSl: 0.8, rrRatio: 3 },
  // モードB: 両方ATR連動
  { mode: "B", atrPeriod: 30, atrMultiplier: 4.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.0 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 4.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.5 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 4.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3.0 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.0 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.5 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 3.0 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 6.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.0 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 6.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.5 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.3, maxSl: 1.0, rrRatio: 2.0 },
  { mode: "B", atrPeriod: 30, atrMultiplier: 5.0, minSl: 0.3, maxSl: 1.0, rrRatio: 2.5 },
  { mode: "B", atrPeriod: 50, atrMultiplier: 4.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.0 },
  { mode: "B", atrPeriod: 50, atrMultiplier: 4.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.5 },
  { mode: "B", atrPeriod: 50, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.0 },
  { mode: "B", atrPeriod: 20, atrMultiplier: 5.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.0 },
  { mode: "B", atrPeriod: 20, atrMultiplier: 6.0, minSl: 0.3, maxSl: 0.8, rrRatio: 2.0 },
];

console.log(`\n--- スイープ: ${sweepConfigs.length}パターン ---`);
interface Result { params: SweepParams; totalPnl: number; trades: number; winRate: number; stopLosses: number; avgSlPct: number; }
const results: Result[] = [];

for (let i = 0; i < sweepConfigs.length; i++) {
  const params = sweepConfigs[i];
  ...
  results.push({ params, totalPnl, trades, winRate, stopLosses, avgSlPct });
  process.stdout.write(`  ${i+1}/${sweepConfigs.length} done\r`);
}

results.sort((a, b) => b.totalPnl - a.totalPnl);
console.log(`\n\n=== 結果 TOP15 ===`);
console.log(`ベースライン: ${basePnl>=0?"+":""}${Math.round(basePnl).toLocaleString()}円 | ${baseTrades}T | 勝率${((baseWins/(baseWins+baseLosses))*100).toFixed(1)}% | SL${baseSL}`);
console.log("-".repeat(120));
console.log("# | Mode | Per | Multi | MinSL | MaxSL | RR  | PnL          | Trades | WinRate | SL  | AvgSL% | vs Base");
console.log("-".repeat(120));
for (let i = 0; i < Math.min(15, results.length); i++) {
  const r = results[i]; const p = r.params; const diff = r.totalPnl - basePnl;
  console.log(`${String(i+1).padStart(2)} | ${p.mode.padEnd(4)} | ${String(p.atrPeriod).padStart(3)} | ${p.atrMultiplier.toFixed(1).padStart(5)} | ${p.minSl.toFixed(1)}%  | ${p.maxSl.toFixed(1)}%  | ${p.rrRatio.toFixed(1)} | ${(r.totalPnl>=0?"+":"") + Math.round(r.totalPnl).toLocaleString().padStart(10)}円 | ${String(r.trades).padStart(4)}  | ${r.winRate.toFixed(1).padStart(5)}% | ${String(r.stopLosses).padStart(3)} | ${r.avgSlPct.toFixed(3)}% | ${(diff>=0?"+":"")+Math.round(diff).toLocaleString()}`);
}
...
if (results.length > 0) {
  ...
  console.log(`\n=== 最適パラメータ日別詳細 ===`);
  console.log(`params: mode=${best.params.mode} ATR${best.params.atrPeriod}×${best.params.atrMultiplier} SL=${best.params.minSl}〜${best.params.maxSl}% RR=${best.params.rrRatio}`);
  for (const { date, data } of allDayData) {
    const r = simulateDay(data, best.params);
    const rf = simulateDay(data, fixedP);
    console.log(`  ${date}: 動的${r.totalPnl>=0?"+":""}${Math.round(r.totalPnl).toLocaleString()}円(${r.trades}T,SL${r.stopLosses},avgSL${r.slPcts.length>0?(r.slPcts.reduce((s,v)=>s+v,0)/r.slPcts.length).toFixed(3):"N/A"}%) vs 固定${rf.totalPnl>=0?"+":""}${Math.round(rf.totalPnl).toLocaleString()}円(${rf.trades}T,SL${rf.stopLosses}) → ${(r.totalPnl-rf.totalPnl>=0?"+":"")}${Math.round(r.totalPnl-rf.totalPnl).toLocaleString()}円`);
  }
}
