...
// 案A（ブースト09:30〜11:30）+ 案B（出来高ブレイクLONG vol≥1.5x + 直近高値更新）
// 30営業日シミュレーション
...
async function runSim(mode: "current" | "ab") {
  ...
  for (const sym of SYMBOLS) {
    ...
    for (const date of simDates) {
      ...
      for (let i = 0; i < dayCandles.length; i++) {
        ...
        if (mode === "ab") {
          // 案A: 前場ブースト（09:30〜11:30）
          const inBoost = timeMin <= 690; // 〜11:30
          if (inBoost && maDeviation < 1.0 && barBody < 0.5 && bearBars <= 5) canEntry = true;
          // 案B: 出来高ブレイクLONG（全時間帯、大台超え以外）
          if (volRatio >= 1.5) canEntry = true;
          // 現行バイパスも維持
...
async function main() {
  for (const mode of ["current", "ab"] as const) {
    const label = mode === "current" ? "現行（静かな上昇バイパスのみ）" : "案A+B（前場ブースト09:30-11:30 + 出来高ブレイクLONG）";
    ...
    for (const r of ["TP","SL","前場","大引け","EOD"]) {
      const rt = trades.filter(t=>t.exit===r);
