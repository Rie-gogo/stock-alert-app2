/**
 * jq_backtest_6m.ts
 * analysis/jq_data/<symbol>.json (J-Quants 1分足) を読み込み、
 * 半年分（約130営業日）のバックテストを実行する。
 *
 * 月別・銘柄別・決済理由別の詳細集計を行い、60日結果との比較も出力する。
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/jq_backtest_6m.ts
 */
...
function main() {
  ...
  for (const s of TARGET_STOCKS) {
    ...
    if (!fs.existsSync(fp)) { console.warn(`[jq-6m] missing ${s.symbol}.json`); continue; }
  ...
  console.log(`[jq-6m] Days found: ${allDays.length} (${allDays[0]} .. ${allDays[allDays.length - 1]})`);
  console.log(`[jq-6m] SHORT_STOP_LOSS_PERCENT=${SHORT_STOP_LOSS_PERCENT}, LUNCH_EXIT_ALL_MINUTE=${LUNCH_EXIT_ALL_MINUTE}, SUPPRESS_AFTERNOON_ENTRY=${SUPPRESS_AFTERNOON_ENTRY}`);

  const dailyRows: string[] = ["date,month,marketEfficiency,rangeBoundDay,totalProfit,winCount,lossCount,winRate"];
  ...
  const monthAgg = new Map<string, { profit: number; win: number; loss: number; trades: number; days: number; posDays: number }>();
  ...
  for (const day of allDays) {
    ...
    dailyRows.push([day, month, eff.toFixed(3), String(rangeBound), Math.round(dayProfit), dayWin, dayLoss, wr.toFixed(3)].join(","));
    dailyProfits.push({ date: day, profit: Math.round(dayProfit), month });

    // 月別集計
    const ma = monthAgg.get(month) ?? { profit: 0, win: 0, loss: 0, trades: 0, days: 0, posDays: 0 };
    ma.profit += dayProfit; ma.win += dayWin; ma.loss += dayLoss; ma.trades += dayWin + dayLoss; ma.days++;
    ...
    console.log(`[jq-6m] ${day}: profit=${Math.round(dayProfit)} win=${dayWin} loss=${dayLoss} eff=${eff.toFixed(2)} range=${rangeBound}`);
  }

  const outDir = path.join(process.cwd(), "analysis", "jq_out");
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, "daily_6m.csv"), dailyRows.join("\n"), "utf8");
  ...
  fs.writeFileSync(path.join(outDir, "by_symbol_6m.csv"), symRows.join("\n"), "utf8");
  ...
  fs.writeFileSync(path.join(outDir, "by_reason_6m.csv"), reasonRows.join("\n"), "utf8");
  ...
  for (const [m, a] of Array.from(monthAgg.entries()).sort()) {
    ...
    monthRows.push([m, Math.round(a.profit), a.win, a.loss, a.trades, wr.toFixed(3), a.days, a.posDays, posRate.toFixed(3), Math.round(avgPerDay)].join(","));
  }
  fs.writeFileSync(path.join(outDir, "by_month_6m.csv"), monthRows.join("\n"), "utf8");
  ...
  console.log("\n===== J-QUANTS 1m 6-MONTH BACKTEST SUMMARY =====");
  ...
  console.log(`\n===== MONTHLY BREAKDOWN =====`);
  for (const [m, a] of Array.from(monthAgg.entries()).sort()) {
    const wr = a.trades > 0 ? (a.win / a.trades * 100).toFixed(1) : "0.0";
    const avgDay = a.days > 0 ? Math.round(a.profit / a.days) : 0;
    console.log(`  ${m}: profit=${Math.round(a.profit)} avg/day=${avgDay} winRate=${wr}% (${a.days}日 ${a.posDays}勝${a.days - a.posDays}敗)`);
  }
  console.log(`\n[jq-6m] CSVs written to ${outDir}`);
}
