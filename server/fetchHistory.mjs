...
async function callDataApi(apiId, options) {
  ...
  const resp = await fetch(fullUrl, {
    ...
    headers: {
      ...
      "connect-protocol-version": "1", authorization: `Bearer ${FORGE_KEY}`,
    },
...
for (const stock of STOCKS) {
  try {
    const raw = await callDataApi("YahooFinance/get_stock_chart", {
      query: { symbol: stock.ticker, region: "JP", interval: "2m", range: "1mo" },
    });
    const data = raw.jsonData ? JSON.parse(raw.jsonData) : (raw.data ?? raw);
    const result = data?.chart?.result?.[0];
    if (!result) { console.log(`${stock.symbol}: no result`); out[stock.symbol]=null; continue; }
    ...
    for (let i = 0; i < ts.length; i++) {
      ...
      const jstMs = ts[i] * 1000 + 9 * 3600 * 1000;
      const jd = new Date(jstMs);
      const dayKey = jd.toISOString().slice(0, 10);
      const hh = jd.getUTCHours();
      const mm = jd.getUTCMinutes();
      const time = `${String(hh).padStart(2,"0")}:${String(mm).padStart(2,"0")}`;
      if (!byDay[dayKey]) byDay[dayKey] = [];
      byDay[dayKey].push({
        time, timestamp: ts[i]*1000,
        open: q.open[i], high: q.high[i] ?? q.open[i],
        low: q.low[i] ?? q.open[i], close: q.close[i], volume: q.volume[i] ?? 0,
      });
    }
    out[stock.symbol] = { name: stock.name, byDay };
    const days = Object.keys(byDay).filter(d => byDay[d].length >= 30);
    console.log(`${stock.symbol} ${stock.name}: ${ts.length} bars, ${days.length} valid days`);
  } catch (e) {
...
console.log("Saved history candles");
