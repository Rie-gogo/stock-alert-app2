...
interface SimConfig {
  ...
  mediumConditionalAllow: boolean; // mediumでも条件付きで許可するか
  mediumMinBoardScore: number; // medium許可時の最低板読みスコア
  mediumMinADX: number; // medium許可時の最低ADX
  timeFilter: boolean; // 時間帯フィルターを適用するか
  pullbackMin: number; // 押し目深さ最小値
  pullbackMax: number; // 押し目深さ最大値
  highVolSL: number;
  highVolTP: number;
  defaultSL: number;
  defaultTP: number;
...
console.log("medium条件付き許可スイープ（板読みスコア+ADX）");
console.log("（③改+⑤適用済みベース、7日間）");
console.log("=".repeat(80));
console.log("");
...
const CONFIGS: SimConfig[] = [
  // 現行仕様（medium完全禁止）
  { label: "現行仕様（medium完全禁止）", blockMediumDirect: true, mediumConditionalAllow: false, mediumMinBoardScore: 0, mediumMinADX: 0, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // medium条件付き許可: 板≥ 3 + ADX≥25
  { label: "medium許可(板≥3+ADX≥25)", blockMediumDirect: true, mediumConditionalAllow: true, mediumMinBoardScore: 3, mediumMinADX: 25, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // medium条件付き許可: 板≥ 3 + ADX≥30
  { label: "medium許可(板≥3+ADX≥30)", blockMediumDirect: true, mediumConditionalAllow: true, mediumMinBoardScore: 3, mediumMinADX: 30, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // medium条件付き許可: 板≥ 3 + ADX≥35
  { label: "medium許可(板≥3+ADX≥35)", blockMediumDirect: true, mediumConditionalAllow: true, mediumMinBoardScore: 3, mediumMinADX: 35, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // medium条件付き許可: 板≥ 4 + ADX≥25
  { label: "medium許可(板≥4+ADX≥25)", blockMediumDirect: true, mediumConditionalAllow: true, mediumMinBoardScore: 4, mediumMinADX: 25, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // medium条件付き許可: 板≥ 4 + ADX≥30
  { label: "medium許可(板≥4+ADX≥30)", blockMediumDirect: true, mediumConditionalAllow: true, mediumMinBoardScore: 4, mediumMinADX: 30, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // medium条件付き許可: 板≥ 5 + ADX≥25
  { label: "medium許可(板≥5+ADX≥25)", blockMediumDirect: true, mediumConditionalAllow: true, mediumMinBoardScore: 5, mediumMinADX: 25, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // medium条件付き許可: 板≥ 3 + ADX≥20
  { label: "medium許可(板≥3+ADX≥20)", blockMediumDirect: true, mediumConditionalAllow: true, mediumMinBoardScore: 3, mediumMinADX: 20, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
  
  // 参考: medium全許可（改良前）
  { label: "参考: medium全許可（改良前）", blockMediumDirect: false, mediumConditionalAllow: false, mediumMinBoardScore: 0, mediumMinADX: 0, timeFilter: true, pullbackMin: 0.30, pullbackMax: 0.70, highVolSL: 0.5, highVolTP: 1.5, defaultSL: 0.5, defaultTP: 1.5 },
];
...
console.log("");
// 日別比較
console.log("日付       | 現行(完全禁止) | 板3+ADX25  | 板3+ADX30  | 板3+ADX35  | 板4+ADX25  | 参考(全許可)");
console.log("-----------|------------|------------|------------|------------|------------|------------");
for (let i = 0; i < DATES.length; i++) {
  const d = DATES[i];
  const indices = [0, 1, 2, 3, 4, 8]; // 現行, 板3ADX25, 板3ADX30, 板3ADX35, 板4ADX25, 全許可
  const vals = indices.map(idx => {
    const r = results[idx];
    if (!r) return "          N/A";
    const day = r.daily[i];
    return `${(day.pnl >= 0 ? "+" : "") + Math.round(day.pnl).toLocaleString().padStart(7)}円(${day.trades}T)`;
  });
