/**
 * sim_adx_sweep.ts
 * 改良策2: ADXフィルター閾値引き上げのパラメータスイープ。
 * 
 * 現行のdetectSignals()はADX<20でMAクロス系を抑制するが、
 * このスクリプトでは追加のADXフィルターを適用し、
 * ADX<25, <30等の場合にMAクロス系シグナルを追加で抑制する効果を検証する。
 * 
 * 方式: detectSignals()の結果を受け取った後、独自にADXを計算し、
 * ADX < 新閾値 かつ MAクロス系シグナルの場合にエントリーをブロックする。
 * （detectSignals内部のADX<20フィルターは既に適用済みなので、
 *  新閾値>20の場合のみ追加効果がある）
 * 
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/sim_adx_sweep.ts
 */
...
// === ADXフィルター判定 ===
// シグナルのreasonがMAクロス系かどうかを判定
function isGcDcSignal(reason: string | undefined): boolean {
  if (!reason) return false;
  return reason.includes("クロス") || reason.includes("戻り売り");
}
...
// === シミュレーション関数 ===
function simulateDay(dayData: RtCandleRow[], adxThreshold: number): { trades: Trade[]; totalPnl: number } {
  ...
  for (const candle of sorted) {
    ...
    // シグナル検出（detectSignals内部でADX<20フィルターは適用済み）
    ...
    // === 追加ADXフィルター ===
    // adxThreshold > 20 の場合、detectSignals内部のADX<20フィルターを通過したシグナルに対して
    // さらにADX < adxThreshold のMAクロス系シグナルをブロックする
    if (latestSignal.signal && adxThreshold > 20) {
      const reason = latestSignal.signal.reason;
      if (isGcDcSignal(reason)) {
        // ADXを計算
        const highs = buffer.map(c => c.high);
        const lows = buffer.map(c => c.low);
        const cls = buffer.map(c => c.close);
        const adxSeries = calcADX(highs, lows, cls);
        const adxVal = adxSeries[adxSeries.length - 1];
        if (adxVal === null || adxVal < adxThreshold) {
          // ADXが閾値未満 → このシグナルをブロック
...
console.log("改良策2: ADXフィルター閾値スイープ（B・C適用版ベース、6日間、6/23除外）");
console.log("=".repeat(80));
console.log("");
console.log("現行: ADX < 20 でMAクロス系シグナルを抑制");
console.log("検証: ADX閾値を20〜40の範囲でスイープし、最適値を探索");
console.log("対象シグナル: ゴールデンクロス、デッドクロス、VWAPクロス上抜け/下抜け、戻り売り");
console.log("");

// スイープ閾値
const ADX_THRESHOLDS = [20, 22, 25, 27, 30, 33, 35, 40];

// データプリロード
const allDayData = new Map<string, RtCandleRow[]>();
for (const date of DATES) { allDayData.set(date, loadDayData(date)); }

interface SweepResult { threshold: number; totalPnl: number; trades: number; wins: number; losses: number; slCount: number; daily: { date: string; pnl: number; trades: number; wins: number; losses: number }[]; blockedSignals: number; }

const results: SweepResult[] = [];

for (const threshold of ADX_THRESHOLDS) {
  let totalPnl = 0, totalTrades = 0, totalWins = 0, totalLosses = 0, totalSL = 0;
  const daily: { date: string; pnl: number; trades: number; wins: number; losses: number }[] = [];
  
  for (const date of DATES) {
    const dayData = allDayData.get(date)!;
    if (dayData.length === 0) { daily.push({ date, pnl: 0, trades: 0, wins: 0, losses: 0 }); continue; }
    const result = simulateDay(dayData, threshold);
    ...
    daily.push({ date, pnl: result.totalPnl, trades: closed.length, wins, losses });
  }
  
  results.push({ threshold, totalPnl, trades: totalTrades, wins: totalWins, losses: totalLosses, slCount: totalSL, daily, blockedSignals: 0 });
  const wr = totalTrades > 0 ? Math.round(totalWins / totalTrades * 100) : 0;
  console.log(`ADX閾値=${String(threshold).padStart(2)}: ${totalPnl >= 0 ? "+" : ""}${Math.round(totalPnl).toLocaleString().padStart(10)}円 (${totalTrades}T, W${totalWins}/L${totalLosses}, 勝率${wr}%, SL${totalSL})`);
}
...
const baseline = results[0]; // ADX=20がベースライン
const sorted2 = [...results].sort((a, b) => b.totalPnl - a.totalPnl);

console.log("順位 | ADX閾値 | 合計損益      | 差分         | 取引 | 勝率  | SL回数 | 平均損益/取引");
console.log("-----|---------|--------------|-------------|------|-------|--------|-------------");
for (let i = 0; i < sorted2.length; i++) {
  const r = sorted2[i];
  const diff = r.totalPnl - baseline.totalPnl;
  const wr = r.trades > 0 ? Math.round(r.wins / r.trades * 100) : 0;
  const avgPnl = r.trades > 0 ? Math.round(r.totalPnl / r.trades) : 0;
  console.log(`${String(i+1).padStart(4)} | ${String(r.threshold).padStart(5)}   | ${(r.totalPnl >= 0 ? "+" : "") + Math.round(r.totalPnl).toLocaleString().padStart(10)}円 | ${(diff >= 0 ? "+" : "") + Math.round(diff).toLocaleString().padStart(9)}円 | ${String(r.trades).padStart(4)} | ${String(wr).padStart(3)}%  | ${String(r.slCount).padStart(4)}   | ${(avgPnl >= 0 ? "+" : "") + avgPnl.toLocaleString()}円`);
}
...
console.log("ADX閾値 | " + DATES.map(d => d.slice(5)).join("    | ") + "    | 合計");
console.log("--------|" + DATES.map(() => "------------|").join("") + "------------");
for (const r of results) {
  const cols = r.daily.map(d => `${d.pnl >= 0 ? "+" : ""}${Math.round(d.pnl/1000)}k(${d.trades}T)`);
  console.log(`  ${String(r.threshold).padStart(2)}    | ${cols.map(c => c.padStart(10)).join(" | ")} | ${r.totalPnl >= 0 ? "+" : ""}${Math.round(r.totalPnl).toLocaleString()}円`);
}

// ブロックされたシグナルの分析（ADX=25の場合）
console.log("");
console.log("=".repeat(80));
console.log("ADX=25 詳細分析: ブロックされたシグナルの内訳");
console.log("=".repeat(80));
console.log("");

// ADX=25でブロックされるシグナルを特定するため、ADX=20とADX=25の取引を比較
const tradesAt20: Trade[] = [];
const tradesAt25: Trade[] = [];
for (const date of DATES) {
  const dayData = allDayData.get(date)!;
  if (dayData.length === 0) continue;
  const r20 = simulateDay(dayData, 20);
  const r25 = simulateDay(dayData, 25);
  tradesAt20.push(...r20.trades);
  tradesAt25.push(...r25.trades);
}

const entries20 = tradesAt20.filter(t => t.action === "buy" || t.action === "short");
const entries25 = tradesAt25.filter(t => t.action === "buy" || t.action === "short");
console.log(`エントリー数: ADX=20: ${entries20.length}回 → ADX=25: ${entries25.length}回 (${entries25.length - entries20.length}回)`);

// ADX=20にあってADX=25にないエントリーを特定
const entryKeys25 = new Set(entries25.map(t => `${t.date}_${t.time}_${t.sym}`));
const blockedEntries = entries20.filter(t => !entryKeys25.has(`${t.date}_${t.time}_${t.sym}`));
console.log(`ブロックされたエントリー: ${blockedEntries.length}件`);
if (blockedEntries.length > 0) {
  console.log("");
  console.log("ブロックされたエントリー詳細:");
  for (const t of blockedEntries) {
    // 対応する決済を探す
    const exitTrade = tradesAt20.find(et => et.sym === t.sym && et.date === t.date && (et.action === "exit" || et.action === "forced_close") && et.time > t.time);
    const pnl = exitTrade?.pnl ?? 0;
    console.log(`  ${t.date} ${t.time} ${t.sym} ${t.action} ${t.reason} → ${pnl >= 0 ? "+" : ""}${Math.round(pnl).toLocaleString()}円`);
  }
}
