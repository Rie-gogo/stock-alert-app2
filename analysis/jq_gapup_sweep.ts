/**
 * jq_gapup_sweep.ts
 * ギャップアップ時のショート禁止フィルターのスイープ
 *
 * 仮説: 当日始値が前日終値より+X%以上高い日はショート禁止
 * → ギャップアップ日は上昇モメンタムが強く、空売りは不利
 * → 1月の損失（-120,950円）の主因であるギャップアップ日の損失を防ぐ
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/jq_gapup_sweep.ts
 */
...
function runBacktest(
  ...
  for (const day of allDays) {
    ...
    for (const s of TARGET_STOCKS) {
      ...
      // 前日終値を取得
      let prevDayClose: number | undefined = undefined;
      if (prevDay && overrides.gapUpShortBlockPercent !== undefined) {
        const prevBars = byTicker.get(s.symbol)?.get(prevDay);
        if (prevBars && prevBars.length > 0) {
          const sortedPrev = [...prevBars].sort((a, b) => a.Time.localeCompare(b.Time));
          prevDayClose = sortedPrev[sortedPrev.length - 1].C;
        }
      }

      const effectiveOverrides: SimOverrides = { ...overrides, prevDayClose };
      const res = simulateStockReal(s.symbol, s.ticker, s.name, candles, marketBiasByProgress, 3_000_000, 70, 30, 2.0, rangeBound, 1.0, effectiveOverrides);
      if (!res) continue;
...
function main() {
  ...
  // ギャップアップ閾値のスイープ（%単位）
  // undefined = フィルターなし（ベースライン）
  const thresholds: (number | undefined)[] = [undefined, 3.0, 2.0, 1.5, 1.0, 0.5];

  const results: { threshold: number | undefined; result: ReturnType<typeof runBacktest> }[] = [];

  for (const thresh of thresholds) {
    const label = thresh === undefined ? "なし(ベース)" : `${thresh}%`;
    process.stdout.write(`[sweep] gapUpBlock=${label}... `);
    const overrides: SimOverrides = thresh === undefined ? { ...BASE } : { ...BASE, gapUpShortBlockPercent: thresh };
  ...
  console.log("\n===== ギャップアップフィルタースイープ結果 =====");
  console.log(`ベースライン(なし): ${baseline.toLocaleString()}円`);
  for (const { threshold, result } of results) {
    const label = threshold === undefined ? "なし" : `${threshold}%`;
    const improvement = result.totalProfit - baseline;
    const avg = Math.round(result.totalProfit / result.days);
    const sign = improvement >= 0 ? "✅" : "❌";
    console.log(`${sign} ${label}: ${result.totalProfit.toLocaleString()}円 (${improvement >= 0 ? '+' : ''}${improvement.toLocaleString()}円) 日平均${avg.toLocaleString()}円 pos=${result.posDays}日 neg=${result.negDays}日`);
  }

  // 月別詳細表示
  console.log("\n===== 月別損益（閾値別） =====");
  const months = Array.from(results[0].result.monthAgg.keys()).sort();
  const header = ["月", "なし(ベース)", ...results.slice(1).map(r => `${r.threshold}%`)].join("\t");
  console.log(header);
