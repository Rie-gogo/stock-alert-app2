...
const sourceAuditIt = process.env.RUN_SOCIONEXT_SOURCE_AUDIT === "1" ? it : it.skip;

describe("6526確認型LONG Git fixtureソース監査", () => {
  sourceAuditIt("DBの最新ID重複除去済み46日15,079足とfixtureが完全一致する", async () => {
    ...
    const [rawRows] = await connection.query(`
      ...
      INNER JOIN (
        ...
        FROM rt_candles
        WHERE symbol = '6526' AND tradeDate <= '2026-08-28'
    ...
    const source = {
      ...
      segments: Array.from(byDate.entries()).map(([tradeDate, candles]) => ({
