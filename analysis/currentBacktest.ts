/**
 * currentBacktest.ts
 * 現在の最良設定（ロング1.5%、ショート0.55%、昼休み11:20）でバックテストを実行し
 * 最悪日を特定する。
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/currentBacktest.ts
 */
...
function main() {
  ...
  let grandTotal = 0;
  const dailyResults: { date: string; profit: number }[] = [];
  ...
  for (const day of allDays) {
    ...
    dailyResults.push({ date: day, profit: Math.round(dayProfit) });
  }

  // 最悪日トップ10
  const sorted = [...dailyResults].sort((a, b) => a.profit - b.profit);
  console.log("\n====== 現在設定（ロング1.5%）での最悪日トップ10 ======");
  for (const r of sorted.slice(0, 10)) {
    console.log(`  ${r.date}: ${r.profit.toLocaleString()}円`);
  }
  console.log(`\n総損益: ${Math.round(grandTotal).toLocaleString()}円`);
  console.log(`日平均: ${Math.round(grandTotal / dailyResults.length).toLocaleString()}円`);
}
