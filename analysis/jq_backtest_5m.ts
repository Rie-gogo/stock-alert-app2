/**
 * jq_backtest_5m.ts
 * 元金500万円でのバックテスト（半年137営業日）
 * jq_backtest_6m.ts の元金を 3,000,000 → 5,000,000 に変更したもの。
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/jq_backtest_5m.ts
 */
...
const INITIAL_CAPITAL = 5_000_000; // ← 500万円
...
function main() {
  ...
  for (const s of TARGET_STOCKS) {
    ...
    if (!fs.existsSync(fp)) { console.warn(`[jq-5m] missing ${s.symbol}.json`); continue; }
  ...
  console.log(`[jq-5m] Days: ${allDays.length} (${allDays[0]} .. ${allDays[allDays.length - 1]})`);
  console.log(`[jq-5m] INITIAL_CAPITAL=${INITIAL_CAPITAL.toLocaleString()}円`);

  const dailyRows: string[] = ["date,month,totalProfit,winCount,lossCount,winRate"];
  ...
  const dailyProfits: { date: string; profit: number; month: string }[] = [];
  ...
  for (const day of allDays) {
    ...
    for (const s of TARGET_STOCKS) {
      ...
      // ← 元金を5,000,000に変更
      const res = simulateStockReal(s.symbol, s.ticker, s.name, candles, marketBiasByProgress, INITIAL_CAPITAL, 70, 30, 2.0, rangeBound, 1.0, { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitAllMinute: LUNCH_EXIT_ALL_MINUTE });
      if (!res) continue;
    ...
    dailyRows.push([day, month, Math.round(dayProfit), dayWin, dayLoss, wr.toFixed(3)].join(","));
    dailyProfits.push({ date: day, profit: Math.round(dayProfit), month });
    ...
    console.log(`[jq-5m] ${day}: profit=${Math.round(dayProfit)} win=${dayWin} loss=${dayLoss}`);
  }

  const outDir = path.join(process.cwd(), "analysis", "jq_out");
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, "daily_5m.csv"), dailyRows.join("\n"), "utf8");
  ...
  fs.writeFileSync(path.join(outDir, "by_symbol_5m.csv"), symRows.join("\n"), "utf8");

  const monthRows = ["month,profit,win,loss,trades,winRate,days,posDays,posRate,avgPerDay"];
  for (const [m, a] of Array.from(monthAgg.entries()).sort()) {
    const wr = (a.win + a.loss) > 0 ? a.win / (a.win + a.loss) : 0;
    const posRate = a.days > 0 ? a.posDays / a.days : 0;
    const avgPerDay = a.days > 0 ? a.profit / a.days : 0;
    monthRows.push([m, Math.round(a.profit), a.win, a.loss, a.win + a.loss, wr.toFixed(3), a.days, a.posDays, posRate.toFixed(3), Math.round(avgPerDay)].join(","));
  }
  fs.writeFileSync(path.join(outDir, "by_month_5m.csv"), monthRows.join("\n"), "utf8");
  ...
  const daysOver25k = profitsArr.filter(p => p >= 25000).length;
  const sorted = [...profitsArr].sort((a, b) => a - b);
  const medianDay = sorted[Math.floor(sorted.length / 2)];

  console.log("\n===== 元金500万円 半年バックテスト SUMMARY =====");
  console.log(`Traded days: ${tradedDays}`);
  console.log(`Total profit: ${Math.round(grandTotal).toLocaleString()} yen`);
  console.log(`Avg/day: ${Math.round(avg).toLocaleString()} yen`);
  console.log(`Median/day: ${medianDay.toLocaleString()} yen`);
  ...
  console.log(`Best day: ${best.toLocaleString()}, Worst day: ${worst.toLocaleString()}`);
  console.log(`Days >= 15,000 yen: ${daysOver15k}/${tradedDays} (${(daysOver15k / tradedDays * 100).toFixed(1)}%)`);
  console.log(`Days >= 25,000 yen: ${daysOver25k}/${tradedDays} (${(daysOver25k / tradedDays * 100).toFixed(1)}%)`);
  console.log(`\n===== MONTHLY BREAKDOWN =====`);
  for (const [m, a] of Array.from(monthAgg.entries()).sort()) {
    const wr = (a.win + a.loss) > 0 ? (a.win / (a.win + a.loss) * 100).toFixed(1) : "0.0";
    const avgDay = a.days > 0 ? Math.round(a.profit / a.days) : 0;
    console.log(`  ${m}: profit=${Math.round(a.profit).toLocaleString()} avg/day=${avgDay.toLocaleString()} winRate=${wr}% (${a.days}日 ${a.posDays}勝${a.days - a.posDays}敗)`);
  }
  console.log(`\n[jq-5m] CSVs written to ${outDir}`);
}
