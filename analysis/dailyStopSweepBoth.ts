/**
 * dailyStopSweepBoth.ts
 * デイリーストップ（停止ライン × 利益保護ライン）を「上昇相場」と「下落相場」の両方で一括スイープ。
 *
 *  - 上昇相場: 本番 simulateStockReal（1分足）で直近5営業日を再現
 *  - 下落相場: 5分足圧縮の検証ロジック（downDayValidatorと同一）で5/14,5/15,5/19,5/27を再現
 *
 * 各日について建玉イベント(PerStockTrades)を一度だけ生成し、同じイベント列に
 * applyPortfolioRules(config) を設定違いで適用する（シミュレーション固定でストップ設定だけ純粋比較）。
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/dailyStopSweepBoth.ts
 */
...
function evalConfig(days: DayPerStock[], cfg: PortfolioConfig){
  let total=0,worst=Infinity,stops=0,wins=0,losses=0;
  for(const d of days){ const pf=applyPortfolioRules(d.perStock,cfg); total+=pf.acceptedProfit; worst=Math.min(worst,pf.acceptedProfit); wins+=pf.acceptedWins; losses+=pf.acceptedLosses; if(pf.dailyStopTriggered)stops++; }
  return { total, avg: days.length?total/days.length:0, worst, stops, wins, losses };
}
...
async function main(){
  ...
  const lossLimits=[10000,15000,20000,0];
  const profitTargets=[0,15000,20000,30000];

  console.log("\n===== デイリーストップ スイープ（上昇 vs 下落）=====");
  console.log("stopLine\tguard\t|UP合計\tUP平均\tUP最悪\tUP停止\t|DOWN合計\tDOWN平均\tDOWN最悪\tDOWN停止\t|総合計");
  const rows: Array<{ll:number;pt:number;up:ReturnType<typeof evalConfig>;down:ReturnType<typeof evalConfig>;combined:number}>=[];
  for(const ll of lossLimits){
    for(const pt of profitTargets){
      const cfg={...DEFAULT_PORTFOLIO_CONFIG,dailyLossLimit:ll,dailyProfitTarget:pt,momentumAllocation:true};
      const up=evalConfig(upDays,cfg); const down=evalConfig(downDays,cfg);
      const combined=up.total+down.total;
      rows.push({ll,pt,up,down,combined});
      const llLabel=ll===0?"なし":`-${ll/10000}万`; const ptLabel=pt===0?"なし":`+${pt/10000}万`;
      console.log(`${llLabel}\t${ptLabel}\t|${Math.round(up.total)}\t${Math.round(up.avg)}\t${Math.round(up.worst)}\t${up.stops}\t|${Math.round(down.total)}\t${Math.round(down.avg)}\t${Math.round(down.worst)}\t${down.stops}\t|${Math.round(combined)}`);
    }
  }

  const baseline=rows.find(r=>r.ll===0&&r.pt===0)!;
  console.log(`\n[基準] ストップ無し: UP合計=${Math.round(baseline.up.total)} / DOWN合計=${Math.round(baseline.down.total)} / DOWN最悪=${Math.round(baseline.down.worst)}`);
  const candidates=rows.filter(r=>r.up.total>=baseline.up.total*0.97);
  candidates.sort((a,b)=> (b.down.worst - a.down.worst) || (b.combined - a.combined));
  const best=candidates[0];
  console.log(`\n[推奨] stopLine=${best.ll===0?"なし":`-${best.ll/10000}万`} / guard=${best.pt===0?"なし":`+${best.pt/10000}万`}`);
  console.log(`  UP合計=${Math.round(best.up.total)} (基準比${((best.up.total/baseline.up.total)*100).toFixed(0)}%) / DOWN合計=${Math.round(best.down.total)} / DOWN最悪日=${Math.round(best.down.worst)} (基準${Math.round(baseline.down.worst)}から改善) / DOWN停止=${best.down.stops}日`);
}
